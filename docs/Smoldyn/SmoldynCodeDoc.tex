...
Macros are defined near the top of smoldynfuncs.h. The global strings that they use are defined at the top of smolsim.cpp.

\begin{description}
\item[\ttt{\#define CHECK(A)}]
\ttt{if(!(A)) {ErrorType=1;goto failure;} else (void)0}
\hfill \\
...
\item[\ttt{\#define CHECKMEM(A)}]
\ttt{if(!(A)) {ErrorType=3;snprintf(ErrorString,STRCHARLONG,"Cannot allocate memory"); goto failure;} else (void)0}
\hfill \\
Check that memory allocation worked.

\item[\ttt{\#define CHECKS(A,...)}]
\ttt{if(!(A)) {ErrorType=2; snprintf(ErrorString,STRCHARLONG,\_\_VA\_ARGS\_\_); goto failure;} else (void)0}
\hfill \\
This is identical to CHECK, except that it also copies the included string and other arguments to the variable \ttt{ErrorString} if a failure occurs.

\item[\ttt{\#define CHECKM(A,...)}]
\ttt{if(!(A) || strmatherror(ErrorMath,1)) {ErrorType=2; snprintf(strcat(ErrorString,ErrorMath),STRCHARLONG,\_\_VA\_ARGS\_\_); goto failure;} else (void)0}
\hfill \\
This is used after a call to \ttt{strmathsscanf}, which is used for reading numerical values. It works the same as the \ttt{CHECKS} macro, but it also checks for math errors using the \ttt{strmatherror} function and appends those to the end of the error string.

\item[\ttt{\#define CHECKBUG(A,...)}]
\ttt{if(!(A)) {ErrorType=4; snprintf(ErrorString,STRCHARLONG,\_\_VA\_ARGS\_\_); goto failure;} else (void)0}
\hfill \\
This is identical to CHECKS, except for creating a higher level of error type.
...
\item[\ttt{panelptr readpanelname(simptr sim,surfaceptr srf,const char *str)}]
\hfill \\
Reads the name of a panel in \ttt{str}. If this name already includes a surface, using the format $surface:panel$, then \ttt{srf} is ignored. Otherwise, \ttt{srf} is assumed to be the surface that owns the panel. Returns a pointer to the panel, or \ttt{NULL} if the panel wasn't found.

\item[\ttt{int srfcoincidentpanels(panelptr pnl1,panelptr pnl2)}]
\hfill \\
Tests two panels to see if they are essentially identical to each other, meaning defined at the exact same position. Returns 0 if not, 1 if they are completely identical, 2 if their defining points are sequentially rotated (e.g. \ttt{pnl1} has triangle points $a$, $b$, and $c$ and \ttt{pnl2} has triangle points $b$, $c$, $a$), and 3 if the two panels have front and back flipped from each other. Also returns -1 for an error (e.g. the panels have different shapes).
...
% Filament math - filament bending forces
...
% Filament math - filament thermal forces
...
% Filament math - filament dynamics
...
\subsection{Modifications for version 2.76 (not released yet)}
\item Added test for coincident surface panels. This is a new test in \ttt{checksurfaceparams} and a new function \ttt{srfcoincidentpanels}, both in smolsurface.c. This only looks for panels that are identical to each other, including with a rotated point sequence or front/back flipped. It does not detect overlap in only one region of a panel, such as a pair of partially coincident rectangles.
\item Fixed a minor bug in \ttt{strunits}, which created errors if units were used and commands included numbers; the problem was that default units only apply while a configuration file is being read, so there are no default units mid-simulation when a command is being parsed. As a result, Smoldyn couldn't convert the command values. Now, Smoldyn still can't convert values in commands, but this doesn't create an error.
\item Implemented use of \ttt{strmatherror} for error reporting. Before, math errors were generated but never read. I created the \ttt{CHECKM} macro, which behaves the same as the \ttt{CHECKS} macro, except that it also checks for math errors and appends math error strings to the error outputs. It should be used after every call to \ttt{strmathsscanf}. I didn't create a new macro within smolcmd.c, but those calls to \ttt{strmathsscanf} also check for math errors now.
