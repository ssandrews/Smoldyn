...
Filaments are 1-dimensional structures that are designed to represent cytoskeletal filaments, such as actin or microtubules, or DNA. They are intended to be much longer than they are wide. Essentially all of the functions here are designed for filaments in 3D space. However, they also work for 2D filaments, provided that the filament parameters are set up so that the filament always stays within the 2D plane.
...
The filament biology enumeration is used to describe what kind of filament this is, which can then be used to pre-populate many of the filament parameters. The filament dynamics enumeration describes what algorithms to use for its dynamics. They are summarized below; this list will be expanded.
...
Segments start by listing the filament that owns them. Then they have their own position within the filament in \ttt{index}; note that the segment array within a filament is shifted by \ttt{frontseg}, so a pointer to the same segment would be found with \ttt{segment->fil->segments[segment->index+segment->fil->frontseg]} and the result would be identical to \ttt{segment}.
...
Next, \ttt{maxseg} and \ttt{nseg} give the allocated and actual numbers of segments. These are stored in an array, where the index of the front is in \ttt{frontseg}. This is not a circular array, which is more complicated to use, but allows for the front index to be non-zero because this leads to simpler and faster operation if there is regular assembly and/or disassembly. Segments are stored in \ttt{segments}. The segment endpoints are stored in \ttt{nodes}, which is allocated to be \ttt{maxseg+1} long by 3 elements wide for the $x$, $y$, and $z$ values.
...
\subsection*{Filament math - direction cosine matrices}

Following are equations for the filament relative angles ($\bm{A}$, \ttt{dcm}), absolute angles ($\bm{B}$, \ttt{adcm}), and positions ($\bm{x}$, \ttt{xyz}). See also Goldstein, Andrews, 2004 and Andrews, 2014. Filament bending is described by the Tait-Bryan convention:
\begin{longtable}[c]{lcc}
name & symbol & perpendicular axis \\
\hline
yaw & $\phi$ & $z$ \\
pitch & $\theta$ & $y$ \\
roll & $\psi$ & $x$
...
Consider passive direction cosine matrix (dcm) $\bm{\Phi}$, which can be expanded as
\begin{equation}
...
\end{equation}
This dcm converts between two frames of reference. Here, $X$, $Y$, and $Z$ are the unit vectors of the ``lab frame'', meaning the absolute coordinates of the simulation, and $x$, $y$, and $z$ are the unit vectors of the ``molecule frame'', meaning the local coordinates of a filament segment. The dcm expresses the dot products of these unit vectors. As a result, each column of the matrix gives the lab frame coordinates of each unit vector of the molecule frame, and each row of the matrix gives the molecule frame coordinates of each unit vector of the lab frame. It is unitary, meaning that all its eigenvalues equal 1, and its transpose is its inverse, so $\bm{\Phi}^T \bm{\Phi} = \bm{1}$, where $\bm{1}$ is the identity matrix. Left-multiplying a molecule-frame vector by this dcm returns the same vector but now expressed in the lab frame. For example, suppose $\bm{B}$ is a dcm (called an absolute one here) and $\bm{p}$ is a column vector in the molecule frame; a particularly simple vector is $\bm{p}^T=[1,0,0]$, which is the direction parallel to a filament segment. Also, $\bm{p}_L$ is the same vector but expressed in the lab frame. These vectors can be converted back and forth with
\begin{align}
\bm{p}_L &= \bm{B} \cdot \bm{p} \hspace{2cm} \bm{p} = \bm{B}^T\cdot \bm{p}_L \\
\bm{p}_L^T &= \bm{p}^T \cdot \bm{B}^T \hspace{1.5cm} \bm{p}^T = \bm{p}_L^T \cdot \bm{B}
\end{align}
The Sphere.c library has functions for these multiplications.

The transpose of the dcm listed above performs an active rotation of a vector in a constant coordinate system, in which case it's often called a rotation matrix. Here is the transpose of the dcm given in eq. \ref{eq:DirectionCosineMatrix}.
\begin{equation}
\bm{\Phi}^T = \left[ \begin{array}{ccc} \Phi_{Xx} & \Phi_{Yx} & \Phi_{Zx} \\ \Phi_{Xy} & \Phi_{Yy} & \Phi_{Zy} \\ \Phi_{Xz} & \Phi_{Yz} & \Phi_{Zz} \end{array} \right]
\label{eq:DirectionCosineMatrix}
\end{equation}
Now, $X$, $Y$, and $Z$ represent the vector direction before rotation and $x$, $y$, and $z$ represent the vector after rotation. Left-multiplying the rotation matrix on a column vector yields the rotated version of the vector. For example, if $\bm{p}_L$ is a vector before rotation and $\bm{B}^T$ is the active rotation matrix, then it's shown above that $\bm{p} = \bm{B}^T \cdot \bm{p}_L$, in agreement with this description.

For a filament, the vector for segment 0 in its own coordinate system lies along direction $\hat{x} = [1,0,0]^T$. In the lab coordinate system, this same vector is $\bm{B}_0 \hat{x}$. For convenience, $\bm{A}_0$ is defined to be the same dcm,
$$\bm{B}_0 = \bm{A}_0$$
To convert from the segment 1 coordinate system to the segment 0 coordinate system, the coordinates need to be rotated by $\bm{A}_1$. Thus, the segment 1 direction in lab coordinates is
$$\bm{B}_1 \hat{x} = \bm{B}_0 \bm{A}_1 \hat{x}$$
Clearly, the $\hat{x}$ terms can be removed to give the coordinate transformation. This equation contradicts what I wrote in my 2014 paper as well as what I've been assuming all along, which was that the matrices should be in the opposite order, but I believe that it's shown correctly here. This pattern continues with additional segments, so
$$\bm{B}_i = \bm{B}_{i-1} \bm{A}_i = \bm{B}_0 \bm{A}_1 \cdots \bm{A}_{i-1} \bm{A}_i$$
In terms of active rotations, the orientation of segment 0 is rotated by $\bm{B}^T_0$ away from the lab frame. The orientation of segment 1 is rotated away from the lab frame first by $\bm{B}^T_0$ and then by $\bm{A}^T_1$. This continues, so that
$$\bm{B}_i^T = \bm{A}_i^T \bm{B}_{i-1}^T = \bm{A}_i^T \bm{A}_{i-1}^T \cdots \bm{A}_1^T \bm{B}_0^T$$
Computations can also go from the back of the filament toward the front with
$$\bm{B}_i = \bm{B}_{i+1} \bm{A}^T_{i+1}$$
$$\bm{B}_i^T = \bm{A}_{i+1} \bm{B}^T_{i+1}$$

Here's the dcm in more detail, now shown for passive rotation matrix $\bm{A}$ as a product of yaw, pitch, and roll passive rotations.
\begin{align*}
\bm{A} &= \bm{X}(\psi) \bm{Y}(\theta) \bm{Z}(\phi) \\
...
\subsection*{Filament math - filament forces}

For filament forces from bending angles, consider a pair of segments, labeled as $i-1$ and $i$; their vectors, pointing from front to back as normal, are $\bm{x}_{i-1}$ and $\bm{x}_i$. Their nodes are $i-1$ for the front-most node, $i$ for the middle node, and $i+1$ for the back-most node. The absolute passive dcm for the first segment is $\bm{B}_{i-1}$ and the relative passive dcm for the second segment is $\bm{A}_i$.
...
\begin{equation}
\bm{\tau}_{i} = \boldsymbol{x}_i \times \bm{F}_{i+1}.
\label{eq:torque}
\end{equation}
This torque is necessarily perpendicular to $\bm{x}_i$ (and $\bm{F}_{i+1}$) but may not be perpendicular to $\bm{x}_{i-1}$. Alternatively, the second segment could be fixed and one applies force $\bm{F}_{i-1}$ to the first node, which would create torque at the center node of
\begin{equation}
\bm{\tau}_{i}' = \boldsymbol{x}_{i-1} \times \bm{F}_{i-1}
\label{eq:torque2}
\end{equation}
In practice, the center node exerts a torque from effective yaw, pitch, and roll springs, which then creates forces at the end nodes. Inverting these equations (see Wikipedia ``Cross product'') gives the forces on the nodes that arise from the torques. This inversion is not mathematically unique but can also have components that are parallel to the segments, but those are omitted here because we know from the physical situation that the induced forces must be perpendicular to the segment vectors. The forces are
\begin{align*}
\bm{F}_{i+1} &= \frac{\bm{\tau}_{i} \times \bm{x}_i}{| \bm{x}_i|^2} \\
\bm{F}_{i-1} &= \frac{\bm{\tau}_{i}' \times \bm{x}_{i-1}}{| \bm{x}_{i-1}|^2}
\end{align*}
Because the $\bm{\tau}_i$ and $\bm{\tau}'_i$ torques are created from the same yaw, pitch, and roll springs, they are equal to each other but have opposite signs, so
$$\bm{\tau}_i' = -\bm{\tau}_i$$

In addition to creating forces at the end nodes, the torque created at node $i$ by the springs also induces torques into the two segments around their axes. These segment torques are given as $\tau_{i-1}$ for the first segment and $\tau_i$ for the second segment (no bold face because these are scalars; note that $\tau_i \neq \bm{\tau}_i$ because they refer to different things). These induced torques are the projection of the spring torque onto the segment axes,
\begin{align*}
\tau_{i} &= \frac{\bm{\tau}_i \cdot \bm{x}_i}{|\bm{x}_i|} \\
\tau_{i-1} &= \frac{\bm{\tau}_i' \cdot \bm{x}_{i-1}}{|\bm{x}_{i-1}|}
\end{align*}
...
Considering the yaw spring first, it creates torques
...
$$\bm{x}_{i-1} = \frac{\hat{x}}{|\bm{x}_{i-1}|}$$
$$\bm{x}_{i} = \frac{\bm{A}_i^T \hat{x}}{|\bm{x}_{i}|}$$
Also, $\Delta \phi_i = (\phi_i-\phi \degree)$ is the yaw spring displacement. Similarly, $\Delta \theta_i = (\theta_i-\theta \degree)$ and $\Delta \psi_i = (\psi_i-\psi \degree)$. This yaw spring torque creates forces
\begin{align*}
\bm{F}_{i+1} &= \frac{- k_\phi \Delta \phi_i \hat{z} \times \bm{x}_i}{| \bm{x}_i|^2}
= \frac{- k_\phi \Delta \phi_i}{| \bm{x}_i|} \hat{z} \times (\bm{A}_i^T \hat{x})  \\
\bm{F}_{i-1} &= \frac{k_\phi \Delta \phi_i \hat{z} \times \bm{x}_{i-1}}{| \bm{x}_{i-1}|^2}
= \frac{k_\phi \Delta \phi_i}{| \bm{x}_{i-1}|}\hat{z} \times \hat{x}
\end{align*}
These forces are helpful to think about, but turn out not to be useful directly. Instead, we'll add up all the torques first.

The pitch spring creates torques
$$\bm{\tau}_{\theta,i} = - k_\theta \Delta \theta_i \bm{Z}^T(\phi_i) \hat{y} = -\bm{\tau}_{\theta,i}',$$
where $\bm{Z}^T(\phi_i)$ represents the yaw active rotation matrix. It's here because yaw is performed first, and then pitch, so pitch deviations create a torque that's along the rotated $y$-axis. The roll spring creates torques
$$\bm{\tau}_{\psi,i} = - k_\psi \Delta \psi_i \bm{Y}^T(\theta_i) \bm{Z}^T(\phi_i) \hat{x} = -\bm{\tau}_{\psi,i}',$$
where $\bm{Y}^T(\theta_i)$ represents the pitch active rotation matrix. It's here because yaw is performed first, then pitch, and then roll, so roll deviations create a torque that's along the rotated $x$-axis.

Adding up these torques leads to (math is in Sphere.nb, from the Sphere.c library)
\begin{align}
...
\label{eq:BendTorque}
\end{align}
In the vector form, the terms are arranged so that the dominant one for small bending angles is listed first. This induces forces and segment torques
\begin{align*}
\bm{F}_{i+1} &= \frac{\bm{\tau}_{i} \times (\bm{A}_i^T \hat{x})}{| \bm{x}_i|} \\
\bm{F}_{i-1} &= -\frac{\bm{\tau}_{i} \times \hat{x}}{| \bm{x}_{i-1}|} \\
\bm{F}_{i} &= -(\bm{F}_{i+1}+\bm{F}_{i-1}) \\
\tau_{i} &= \bm{\tau}_{i} \cdot (\bm{A}_i^T \hat{x}) \\
\tau_{i-1} &= -\bm{\tau}_{i} \cdot \hat{x}
\end{align*}
These force vectors are in the frame of reference of the $i-1$ segment. Pre-multiply them by $\bm{B}_{i-1}$ to convert to absolute coordinates. Alternatively, the torque, $\bm{\tau}_i$ can be rotated to an absolute frame first, and then it's probably easiest to just multiply by the original segment vectors. In an absolute reference frame:
\begin{align*}
\bm{F}_{i+1} &= \frac{(\bm{B}_{i-1} \bm{\tau}_{i}) \times \bm{x}_i}{| \bm{x}_i|^2} \\
\bm{F}_{i-1} &= -\frac{(\bm{B}_{i-1} \bm{\tau}_{i}) \times \bm{x}_{i-1}}{| \bm{x}_{i-1}|^2} \\
\bm{F}_{i} &= -(\bm{F}_{i+1}+\bm{F}_{i-1}) \\
\tau_{i} &= \frac{(\bm{B}_{i-1} \bm{\tau}_{i}) \cdot \bm{x}_i }{| \bm{x}_i|} \\
\tau_{i-1} &= -\frac{(\bm{B}_{i-1} \bm{\tau}_{i}) \cdot \bm{x}_{i-1}}{| \bm{x}_{i-1}|}
\end{align*}

A caveat with these equations is that the node $i$ springs may induce forces over the entire length of the filament, not only at the nearest nodes. This adds quite a lot of complication that I'll ignore for now.

\subsection*{Filament math - filament dynamics}

These and other forces push on nodes, causing the nodes to move. Also, these and other torques create rotations in the segments. I'm not sure what the motions are, but it's proportional to the forces and torques, and to the time step $\Delta t$. It's also probably inversely proportional to the viscosity, and there's some relationship with the hydrodynamic radius. For the Brownian motion, that can be computed from the fluctuation-dissipation theorem. Anyhow, they move.

After finding new node positions, one needs to compute the new ypr angles. The new roll angles are known from integrating the torques. The new absolute yaw and pitch angles can be computed from
$$\frac{\bm{x}_i}{|\bm{x}_i|} = \bm{B}(\phi,\theta,\psi) \hat{x},$$
where $\bm{x}_i$ is the segment vector for the $i$'th segment in the lab reference frame. The three values of $\bm{x}_i$ are known (which reduces to two independent values after dividing by its length), and $\psi$ is known. This equation needs to be solved for $\phi$ and $\theta$. It expands to
$$\frac{\bm{x}_i}{|\bm{x}_i|} = \left[ \begin{array}{c} x_x \\ x_y \\ x_z \end{array} \right]
= \left[ \begin{array}{c} \textrm{c} \theta \textrm{c} \phi \\ \textrm{s} \psi \textrm{s} \theta \textrm{c} \phi - \textrm{c} \psi \textrm{s} \phi \\ \textrm{c} \psi \textrm{s} \theta \textrm{c} \phi + \textrm{s} \psi \textrm{s} \phi \end{array} \right]$$
Solving for $\phi$ and $\theta$ gives
\begin{align*}
\phi &= \textrm{asin} (x_z \textrm{s} \psi - x_y \textrm{c}\psi) \\
\theta &= \textrm{asin} \left( \frac{x_z \textrm{c} \psi + x_y \textrm{s}\psi}{\textrm{c} \phi} \right)
\end{align*}
...
Local. Converts filament biology type to string. Returns ``none'' for unrecognized biology type. Writes result to \ttt{string} and also returns it directly.

\item[\ttt{enum FilamentBiology filstring2FB(const char *string)}]
Local. Converts filament biology string to enumerated biology type. Returns \ttt{FBnone} for unrecognized input.
...
\item[\ttt{void filUpdateSegmentIndices(filamentptr fil);}]
\hfill \\
Overwrites the segment \ttt{index} element for each segment in a filament to make them correct. Call this, if needed, after adding or removing segments to or from a filament.
...
\hfill \\
Removes one segment or one bead from either the front or back end of a filament. Specify the end in \ttt{endchar} with `f' for front and `b' for back. Returns 0 for success or -1 for a filament that already had no segments. This does not shift the segments within their array.
...
\ttt{viscosity} & medium viscosity & n/a & $(0,\infty)$\\
...
\item[\ttt{int filtypeSetBiology(filamenttypeptr filtype, enum FilamentBiology fb)}]
\hfill \\
Sets the biology for the filament type to \ttt{fb}. NOT WRITTEN YET. This function should also pre-populate as many of the other parameters as possible, which hasn't been done yet. As part of this, I should also allow the user to specify units in the \ttt{sim} structure.
...
\hfill \\
This will run all dynamics for filaments. It's not written yet.
...
\item Lots of work on filaments, including changes to data structures, input syntax, examples, and most functions in smolfilament.c.
