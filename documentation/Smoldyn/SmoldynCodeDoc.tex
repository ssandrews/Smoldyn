...
All compiling options, determining which components to include and what the code is being built for. In the table below, * indicates that this option doesn't work at present. The default values listed below are only the initial settings; several of them get overridden later in this same section, depending on other settings.
\begin{longtable}[c]{lll}
Variable & Description & Default\\
\hline
...
\item[Compiler flags]
This section sets most of the compiler flags for the build. It starts by determining if this is a release or debug build. Other options are `None', `RelWithDebInfo', and `MinSizeRel', but the code doesn't support those options. If it isn't ``Debug'', then it's a release. Depending on this build type, this section sets a list of basic compiler flags, if the compiler is clang or gnu. I tried these flags with Visual Studio, and they don't work there. Next, it creates a list of possible build platforms and goes through the list of possible platforms, adding to the compiler flags as needed as it goes along. As far as this is concerned, Windows and Visual Studio are the same thing. This section also sets the path to the BNG2 perl script.
\begin{longtable}[c]{lll}
Variable & Description & Default\\
\hline
\texttt{CMAKE\_BUILD\_TYPE} & Build type & Release\\
\texttt{COMPILER\_IS\_CLANGXX} & clang C++ compiler\\
