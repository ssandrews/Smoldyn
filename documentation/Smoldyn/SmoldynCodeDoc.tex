...
This file parses the command line arguments using the argparse standard library and then runs the model using \ttt{smoldyn.load\_model()}. The \ttt{load\_model} function might be in utils.py, or maybe that's just another instance of something similar and with the same name.
...
\hfill \\
This command translates a compartment, including dealing with molecule displacements. All of the work is done by \ttt{cmpttranslate}.

\item[\ttt{enum CMDcode cmddiffusecmpt(simptr sim,cmdptr cmd,char *line2);}]
\hfill \\
This command diffuses a compartment, including dealing with molecule displacements. All of the work is done by \ttt{cmpttranslate}.
