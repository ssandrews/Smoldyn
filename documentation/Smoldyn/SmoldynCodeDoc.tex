...
freeglut for Windows is from \url{http://freeglut.sourceforge.net}. I downloaded freeglut 3.2.1 source code and built it on a Windows computer using the Visual Studio compiler. To build, I created and then moved to a build directory, entered ``cmake ..'', and entered ``msbuild freeglut.sln''. This created the files lib/Debug/freeglutd.lib, lib/Debug/freeglut\_staticd.lib, and bin/Debug/freeglutd.dll. The ``d'' characters show that these are the debug versions of the library. I tried all options that I could find and was simply unable to create the non-debug versions. Thus, I duplicated the debug versions and removed the ``d'' from the file name, in that hope that they would work as non-debug versions, too. At present, the Smoldyn build system seems to want the debug versions, so that's fine. I put the compiled libraries in the source/MSVClibs directory, along with a header file from the same freeglut download. Whether the debug or normal versions are wanted depends on the definition of the \texttt{NDEBUG} macro; glut.h calls freeglut\_std.h, and that file has some pragma lines that choose either libfreeglut or libfreeglutd, depending on \texttt{NDEBUG}.

libtiff is from \url{http://download.osgeo.org/libtiff/}. For Mac, I got version 3.9.6, which was the latest of the 3.9 series. Because I didn't want the zlib dependency of libtiff, I configured with ``./configure --disable-zlib", and then entered ``make" and ``sudo make install" as usual. For Windows, I downloaded version 3.8.0, but it required Autotools to compile, which didn't work for me on Windows. So, I downloaded version 4.1.0, which uses CMake. I configured using the option ``cmake .. -DBUILD_SHARED_LIBS=FALSE'' and built with ``msbuild tiff.sln''. This only created the debug libraries, tiffd.lib, tiffxxd.lib, and portd.lib. I'm hoping that only tiffd.lib is needed, and that I can duplicate it to tiff.lib, which is what I did. I copied these libraries over to source/MSVClibs, and also added in several header files (tiff.h, tiffio.h, tiffvers.h, tiffconf.h). The result works.
...
% Chapter: CMake build system code
\chapter{CMake build system code}

\section{Top level CMake file}

The CMake code for Smoldyn, in CMakeLists.txt in the top-most directory, is relatively straightforward, but still causes challenges. This section gives a fairly detailed look at the code, focusing particularly on the variable definitions.

Some aspects of this use somewhat obsolete terms. For example, the ``use\_directories'' statement is used substantially, but this has been replaced by ``target\_use\_directories''.
...
\hline
\texttt{SMOLDYN\_VERSION} & Smoldyn version number & 2.62.dev\\
\texttt{CMAKE\_CXX\_STANDARD} & C++ version number & 17\\
\end{longtable}
...
Variable & Description & Default\\
\hline
\texttt{OPTION\_TARGET\_SMOLDYN} & Create stand-along Smoldyn program & ON\\
\texttt{OPTION\_TARGET\_LIBSMOLDYN} & Create LibSmoldyn library & OFF\\
\end{longtable}

\item[Compiling options]
Most of the compiling options are listed here. They determine which components to include and what the code is being built for. In the table below, * indicates that this option doesn't work at present and so should always be turned off. The default values listed below are only for Mac and Linux and are the initial settings; several of them get overridden later in this same section, depending on other settings.
\begin{longtable}[c]{lll}
Variable & Description & Default\\
\hline
\texttt{OPTION\_VCELL} & Compile Smoldyn for VCell & OFF\\
\texttt{OPTION\_MINGW} & Cross-compile for Windows using MinGW compiler* & OFF\\
\texttt{OPTION\_NSV} & Compile Smoldyn with NextSubvolume functionality & ON\\
\texttt{OPTION\_PDE} & Compile Smoldyn with PDE functionality* & OFF\\
\texttt{OPTION\_VTK} & Compile Smoldyn with VTK functionality & OFF\\
\texttt{OPTION\_STATIC} & Compile Smoldyn with static libraries & OFF\\
\texttt{OPTION\_USE\_OPENGL} & Build with OpenGL support & ON\\
\texttt{OPTION\_USE\_ZLIB} & Build with Zlib support & OFF\\
\texttt{OPTION\_USE\_LIBTIFF} & Whether to include the LibTIFF library & ON\\
\texttt{OPTION\_USE\_ICONV} & Whether to include the Libiconv library & OFF\\
\texttt{OPTION\_PYTHON}  & Build Python module & OFF\\
\texttt{OPTION\_EXAMPLES} & Run Libsmoldyn tests & OFF\\
\texttt{OPTION\_LATTICE} & Whether to build lattice code & depends\\
...
\texttt{MAIN\_FILES} & Name of the main file & smoldyn.c\\
\end{longtable}

\item[Compiler flags]
This section sets most of the compiler flags for the build. It starts by determining what type of build this is, where the options are `Release', `Debug', `None', `RelWithDebInfo', and `MinSizeRel'; it sets the value to `Release' as a default. This is a built-in CMake variable, so CMake defines several compiler flags automatically based on this build type, without them needing to be set here. Next, this section creates a list of possible build platforms and goes through the list of possible platforms, adding to the compiler flags as needed as it goes along. As far as this is concerned, ``MinGW'' means cross-compile from Mac to Windows using the MinGW compiler, and ``Windows'' means compile on Windows using the Visual Studio compiler. This section also sets the path to the BNG2 perl script.
\begin{longtable}[c]{lll}
Variable & Description & Default\\
\hline
\texttt{CMAKE\_BUILD\_TYPE} & Build type & Release\\
\texttt{CMAKE\_C\_FLAGS} & C compiler flags string\\
\texttt{CMAKE\_CXX\_FLAGS} & C++ compiler flags string\\
\texttt{DEP\_LIBS} & Library dependencies for linking\\
\texttt{VCELL\_BUILD} & Build for VCell (ON or OFF)\\
\texttt{APPLE\_BUILD} & Build for Apple (ON or OFF)\\
\texttt{NIX\_BUILD} & Build for Unix or Linux (ON or OFF)\\
\texttt{MINGW\_BUILD} & Build for MinGW (ON or OFF)\\
\texttt{WINDOWS\_BUILD} & Build for Windows (ON or OFF)\\
\texttt{BNG2\_PATH} & Path to BNG2 perl script\\
\end{longtable}

\item[OpenGL (gl and glu, not glut)]
OpenGL is a bit of a challenge to include correctly, and platform dependent. The gl and glu libraries are usually built in, while the glut library is not necessarily, so this section just focuses on the gl and glu libraries. OpenGL is becoming deprecated, but I still use it, so this code starts by stating that it uses legacy preferences. The \texttt{include(FindOpenGL)} statement is used where possible; it automatically defines the variables \texttt{OPENGL\_FOUND}, \texttt{OPENGL\_INCLUDE\_DIR}, and \texttt{OPENGL\_LIBRARIES}. However, the code overrides the include directories for Windows because the automatic system doesn't seem to find the built-in gl.h header file, and a different version of the header file from the Smoldyn directory seems to work well.
\begin{longtable}[c]{lll}
Variable & Description & Purpose\\
\hline
\texttt{HAVE\_OPEN\_GL} & TRUE if available, FALSE if not & smoldynconfig.h\\
\texttt{HAVE\_GL\_H} & TRUE for header file gl.h  & smoldynconfig.h\\
\texttt{HAVE\_GL\_GL\_H} & TRUE for header file GL/gl.h  & smoldynconfig.h\\
\texttt{HAVE\_OPENGL\_GL\_H} & TRUE for header file OpenGL/gl.h  & smoldynconfig.h\\
\texttt{HAVE\_GLU\_H} & TRUE for header file glu.h  & smoldynconfig.h\\
\texttt{HAVE\_GL\_GLU\_H} & TRUE for header file GL/glu.h  & smoldynconfig.h\\
\texttt{HAVE\_OPENGL\_GLU\_H} & TRUE for header file OpenGL/glu.h  & smoldynconfig.h\\
\texttt{OPENGL\_FOUND} & TRUE if OpenGL is found & local only\\
\texttt{OPENGL\_INCLUDE\_DIR} & Include directory & local only\\
\texttt{OPENGL\_LIBRARIES} & OpenGL Library directory & local only\\
\texttt{GLU32\_LIBRARIES} & glu library directory & local only\\
\texttt{DEP\_LIBS} & Library dependencies for linking & Appended\\
\end{longtable}

\item[OpenGL glut]
The OpenGL glut libraries are found and added next. They are standard libraries on Mac and Linux, but aren't included on Windows. As a result, this uses the freeglut library for Windows, using a version provided with the Smoldyn package. From the freeglut documentation, one is supposed to use glut.h if only glut functionality is wanted, and freeglut.h is additional functionality is used that is only supported by freeglut; thus, this should only use glut.h, although parts of it incorrectly use freeglut.h. The \texttt{include(FindGLUT)} statement is used where possible; it automatically defines the variables \texttt{GLUT\_FOUND)}, \texttt{GLUT\_INCLUDE\_DIR}, and \texttt{GLUT\_LIBRARIES}.
\begin{longtable}[c]{lll}
Variable & Description & Purpose\\
\hline
\texttt{HAVE\_GLUT\_H} & TRUE for header file gutl.h  & smoldynconfig.h\\
\texttt{HAVE\_GL\_GLUT\_H} & TRUE for header file GL/gutl.h  & smoldynconfig.h\\
\texttt{HAVE\_GLUT\_GLUT\_H} & TRUE for header file GLUT/gutl.h  & smoldynconfig.h\\
\texttt{HAVE\_GL\_FREEGLUT\_H} & TRUE for header file GL/freeglut.h  & smoldynconfig.h\\
\texttt{GLUT\_FOUND)} & TRUE if glut is found & local only\\
\texttt{GLUT\_INCLUDE\_DIR} & Include directory & local only\\
\texttt{GLUT\_LIBRARIES} & glut library directory & local only\\
\texttt{DEP\_LIBS} & Library dependencies for linking & Appended\\
\end{longtable}
...
\hline
\texttt{HAVE\_LIBTIFF} & TRUE if LibTiff is found, FALSE if not\\
\texttt{TIFF\_INCLUDE\_DIR} & Include directory\\
\texttt{TIFF\_LIBRARY} & Library directory\\
\texttt{DEP\_LIBS} & Appended: library dependencies for linking\\
\end{longtable}
...
Variable & Description\\
\hline
\texttt{HAVE\_ZLIB} & TRUE if Zlib is found, FALSE if not\\
\texttt{ZLIB\_INCLUDE\_DIRS} & Include directory\\
\texttt{ZLIB\_LIBRARIES} & Library directory\\
\texttt{DEP\_LIBS} & Appended: library dependencies for linking\\
\end{longtable}
...
Variable & Description\\
\hline
\texttt{HAVE\_INCONV} & TRUE if Libiconv is found, FALSE if not\\
\texttt{ICONV\_INCLUDE\_DIRS} & Include directory\\
\texttt{ICONV\_LIBRARIES} & Library directory\\
\texttt{DEP\_LIBS} & Appended: library dependencies for linking\\
\end{longtable}
...
Variable & Description\\
\hline
\texttt{VTK\_FOUND} & TRUE if VTK is found, FALSE if not\\
\texttt{VTK\_INCLUDE\_DIRS} & Include directory\\
\texttt{DEP\_LIBS} & Appended: library dependencies for linking\\
\end{longtable}
...
Variable & Description\\
\hline
\texttt{SIGNAL\_H\_DIR} & Directory to signal.h, only for MinGW\\
\texttt{Boost\_INCLUDE\_DIR} & Include directory for Boost, which is in the NSV source\\
\texttt{SRC\_FILES} & Appended: Smoldyn source files\\
\texttt{HEADER\_FILES} & Appended: Smoldyn header files\\
\end{longtable}
...
\end{lstlisting}
\item The first function of smolcmd.c is \texttt{docommand}. In it, add an \texttt{else if} line for the new command. It looks like:
\begin{lstlisting}
