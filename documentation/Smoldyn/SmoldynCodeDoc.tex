...
freeglut for Windows is from \url{https://www.transmissionzero.co.uk/software/freeglut-devel/}. I downloaded the freeglut 3.0.0 MinGW package. I didn't install, but instead put the pre-compiled freeglut files into the Smoldyn package windows directory. \textit{I edited the freeglut file:} In freeglut\_std.h, line 74 is ``pragma comment (lib, "freeglut\_staticd.lib")'' and line 94 is ``pragma comment (lib, "freeglutd.lib")''. However, these debug libraries aren't included in the package, so I removed the ``d'' characters from these library names.
...
This section lists all of the Smoldyn header files and source files, plus a few extra header files from some of the optional features (e.g. vtkwrapper.h). Some of the source files are only included for some of the options. All of the source and main files properties are set to the C++ language. The include directories are set here to all of the directories with header files.
\begin{longtable}[c]{lll}
Variable & Description & Default\\
\hline
...
\item[PDE]
This is an empty section for possible future use for PDE hybrid simulation.
