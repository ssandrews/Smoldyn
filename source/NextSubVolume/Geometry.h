...
public:
	...
	Vect3d get_random_point() {
		Vect3d ret;
		ret[DIM] = this->coord;
		ret[dim_map[DIM][0]] = unirandCCD(low[dim_map[DIM][0]], high[dim_map[DIM][0]]);
		ret[dim_map[DIM][1]] = unirandCCD(low[dim_map[DIM][1]], high[dim_map[DIM][1]]);
		return ret;
	}
	...
	Vect3d get_random_point_triangle() {
		...
		if (U1<0.5) {
			ret[dim_map[DIM][0]] = low[dim_map[DIM][0]] + (high[dim_map[DIM][0]]-low[dim_map[DIM][0]])*sqrt(0.5*U1);
		} else {
			ret[dim_map[DIM][0]] = high[dim_map[DIM][0]] - (high[dim_map[DIM][0]]-low[dim_map[DIM][0]])*sqrt(0.5*(1.0-U1));

		}
		const double U2 = randCCD();
		if (U2<0.5) {
			ret[dim_map[DIM][1]] = low[dim_map[DIM][1]] + (high[dim_map[DIM][1]]-low[dim_map[DIM][1]])*sqrt(0.5*U2);
		} else {
			ret[dim_map[DIM][1]] = high[dim_map[DIM][1]] - (high[dim_map[DIM][1]]-low[dim_map[DIM][1]])*sqrt(0.5*(1.0-U2));
