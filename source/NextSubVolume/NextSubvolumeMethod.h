...
struct HeapNode
{
    HeapNode(double time_at_next_reaction,
...
{
    ReactionsWithSameRateAndLHS(const double rate,
      const ReactionSide& lhs,
      const ReactionSide& rhs)
      : lhs(lhs)
      , rate(rate)
    {
    ...
    //		all_rhs.clear();
    //	}
    bool add_if_same_lhs(const double rate_to_add,
      const ReactionSide& lhs_to_add,
      const ReactionSide& rhs_to_add);
    ReactionSide& pick_random_rhs(const double rand);

    int size() { return all_rhs.size(); }
...
class ReactionList
{
  public:
    ...
    ReactionList(const ReactionList& arg) = default;
    ...
    double get_propensity() { return total_propensity; }
...
class NextSubvolumeMethod
{
  public:
    ...
    void integrate(const double dt) { (*this)(dt); }
    void reset();
    void list_reactions();
    template<typename T>
    ...
    void set_interface_reactions(std::vector<int>& from_indicies,
      std::vector<int>& to_indicies,
      int id,
      const double dt,
      const bool corrected);
    void unset_interface_reactions(std::vector<int>& from_indicies,
      std::vector<int>& to_indicies);
    ...
    void absorption_across(Species& s, T& geometry, const double probability)
    {
        ...
        for (int i = 0; i < n; ++i) {
            const std::vector<int>& neighbrs =
              subvolumes.get_neighbour_indicies(slice[i]);
            ...
            for (int j = 0; j < nn; ++j) {
                if (geometry.lineXsurface(subvolumes.get_cell_centre(slice[i]),
                    ...
                    const double rate = subvolume_reactions[slice[i]].delete_reaction(
                      ReactionEquation(lhs, rhs));
                    if (probability < 1) {
                        subvolume_reactions[slice[i]].add_reaction(
                          rate * (1 - probability), ReactionEquation(lhs, rhs));
                    }
                    if (probability > 0) {
                        ReactionSide rhs_absorb;
                        subvolume_reactions[slice[i]].add_reaction(
                          rate * probability, ReactionEquation(lhs, rhs_absorb));
                    }
                }
                if (geometry.lineXsurface(subvolumes.get_cell_centre(neighbrs[j]),
                    ...
                    const double rate = subvolume_reactions[neighbrs[j]].delete_reaction(
                      ReactionEquation(lhs, rhs));
                    if (probability < 1) {
                        subvolume_reactions[neighbrs[j]].add_reaction(
                          rate * (1 - probability), ReactionEquation(lhs, rhs));
                    }
                    if (probability > 0) {
                        ReactionSide rhs_absorb;
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = 0; j < nn; ++j) {
                if (geometry.lineXsurface(subvolumes.get_cell_centre(slice[i]),
                    ...
                    if (scaling_factor > 0) {
                        subvolume_reactions[slice[i]].add_reaction(
                          rate * scaling_factor, ReactionEquation(lhs, rhs));
                    }
                }
                if (geometry.lineXsurface(subvolumes.get_cell_centre(neighbrs[j]),
                      subvolumes.get_cell_centre(slice[i]))) {
    ...
    template<typename T>
    void add_diffusion_between(Species& s,
      const double rate,
      T& geometry_from,
      T& geometry_to)
    {
    ...
    void clear_reactions(std::vector<int>& cell_indicies)
    {
        ...
        for (std::vector<int>::iterator i = cell_indicies.begin();
             i != cell_indicies.end();
             i++) {
            // subvolume_reactions[i].clear();
            subvolume_reactions[*i].clear();
    ...
    double get_next_event_time() { return heap.top().time_at_next_reaction; }
...
std::ostream&
operator<<(std::ostream& out, NextSubvolumeMethod& b);
}
