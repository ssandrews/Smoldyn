...
char *Zn_vect2csvstring(int *c, int n, char *string)
{
    ...
    if(n > 0)
        snprintf(string, sizeof(string), "%i", c[0]);
    for(i = 1; i < n; i++)
...
int *fprintZV(FILE *stream, int *c, int n)
{
    ...
    if(n)
        fprintf(stream, "%i", c[0]);
    for(i = 1; i < n; i++)
        fprintf(stream, " %i", c[i]);
    fprintf(stream, "\n");
    return c;
}
...
int nextaddZV(int add, int *indx1, int *indx2, int *dim, int rank)
{
    ...
    for(i = rank - 1; i >= 0 && !done; i--) {
        if(add % dim[i] < indx2[i])
            done = 1;
        else
            add /= dim[i];
    }
    if(!done)
        return 1 + indx2addZV(indx2, dim, rank);
    add++;
    for(i += 2; i < rank; i++)
        add = dim[i] * add + indx1[i];
    return add;
}
...
int neighborZV(
    int *indx, int *c, int *dim, int rank, int type, int *wrap, int *mid)
{
    int         i, n, p, ad, k, j;
    ...
    if(type == 0) {
        ...
        for(i = 0; i < p; i++) {
            ...
            for(j = 0; j < rank; j++) {
                ...
                if(ofst[j] < 0 && wrap[j])
                    ofst[j] = dim[j] - 1;
                else if(ofst[j] >= dim[j] && wrap[j])
                    ofst[j] = 0;
                else if(ofst[j] < 0 || ofst[j] >= dim[j])
                    j = rank + 1;
            }
            if(j == rank)
                c[n++] = indx2addZV(ofst, dim, rank);
...
int Zn_permute(int *a, int *b, int n, int k)
{
    ...
    if(n == 0)
        ...
        if(a[1] == a[2] && ans == 1)
            ans = 2;
        else if(a[1] == a[2] && (ans == 4 || ans == 5))
            ans = 0;
        if(a[0] == a[1] && (ans == 2 || ans == 3))
            ans = 4;
        else if(a[0] == a[1] && ans == 5)
            ans = 0;
        if(a[0] == a[2] && (ans == 3 || ans == 4 || ans == 5))
            ans = 0;
    }
