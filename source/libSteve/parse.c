...
ParseFilePtr Parse_AllocFilePtr(const char *fileroot, const char *filename)
{
    ...
    if(fileroot)
        strncpy(pfp->froot, fileroot, STRCHAR - 1);
    if(fileroot)
        strncpy(pfp->fname, fileroot, STRCHAR - 1);
    if(filename)
...
void Parse_FreeFilePtr(ParseFilePtr pfp)
{
    ...
    if(pfp->maxdef) {
        if(pfp->defreplace)
            for(d = 0; d < pfp->maxdef; d++)
                free(pfp->defreplace[d]);
        if(pfp->defkey)
            for(d = 0; d < pfp->maxdef; d++)
                free(pfp->defkey[d]);
    }
...
int Parse_AddDefine(ParseFilePtr pfp, const char *key, const char *replace, int global)
{
    ...
    if(pfp->ndef == pfp->maxdef)
        if(Parse_ExpandDefine(pfp, 1 + 2 * pfp->ndef))
            return 1;
    pfp->ndef++;

    len = strlen(key);
    for(d = 0; d < pfp->ndef - 1; d++) {  // find location
        if(len > strlen(pfp->defkey[d]))
            break;
    }
...
int Parse_DoDefine(ParseFilePtr pfp)
{
    ...
    if(!line2)
        return 0;
    if(!strncmp(line2, "define", 6))
        return 0;
    if(!strncmp(line2, "undefine", 8))
        return 0;
    if(!strncmp(line2, "ifdefine", 8))
        return 0;
    if(!strncmp(line2, "ifundefine", 10))
        return 0;
...
int Parse_CmdLineArg(int *argcptr, char **argv, ParseFilePtr pfp)
{
    ...
    if(argcptr && *argcptr > 0 && argv) {  // there are arguments for reading
        ...
        if(!pfp && maxdefine - ndefine < argc2) {  // allocate space
            ...
            for(; i < newmax; i++)
                CHECK(newkeylist[i] = EmptyString());

            CHECK(newreplist = (char **)calloc(newmax, sizeof(char *)));
            for(i = 0; i < newmax; i++)
                newreplist[i] = NULL;
            for(i = 0; i < maxdefine; i++)
                newreplist[i] = replist[i];
            for(; i < newmax; i++)
                CHECK(newreplist[i] = EmptyString());
...
int Parse_ReadLine(ParseFilePtr *pfpptr, char *word, char **line2ptr, char *erstr)
{
    ...
    if(linetest) {  // pre-process line
        ...
        if(!skip) {
            CHECKS(!toolong, "Line exceeds maximum allowable length of %i characters",
                STRCHAR);
    ...
    if(skip)
        ...
        *pfpptr = pfp;
        ...
        for(d = 0; d < pfp->ndef; d++)
            if(pfp->defgbl[d]) {
                CHECKS(!Parse_AddDefine(pfp1, pfp->defkey[d], pfp->defreplace[d], 1),
