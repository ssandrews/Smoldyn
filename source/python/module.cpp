...
PYBIND11_MODULE(_smoldyn, m)
{
    ...
    py::class_<CallbackFunc>(m, "CallbackFunc")
        .def(py::init<>())
        .def_property("func", &CallbackFunc::getFuncName,
                      &CallbackFunc::setFuncName)
    ...
    py::class_<graphicssuperstruct>(m, "Graphics")
        ...
        .def_property_readonly("diffuseLightColor",
                               [](const graphicssuperstruct &st) {
                                   vector<array<double, 4>> difflight(
                                       MAXLIGHTS);
                                   for (size_t i = 0; i < MAXLIGHTS; i++)
    ...
    py::class_<simstruct>(m, "simstruct")
        ...
        // path")
        .def_readwrite("filepath", &simstruct::filepath,
                       "configuration file path")
        .def_readwrite("filename", &simstruct::filename,
                       "configuration file name")
        .def_readwrite("flags", &simstruct::flags,
                       "command-line options from user")
        .def_readwrite("quitatend", &simstruct::quitatend,
                       "simulation quits at the end")
        ...
        .def_readonly("randseed", &simstruct::randseed,
                      "random number generator seed")
        .def_readonly("eventcount", &simstruct::eventcount,
                      "counter for simulation events")
        .def_readonly("maxvar", &simstruct::maxvar,
                      "allocated user-settable variables")
        .def_readonly("nvar", &simstruct::nvar,
                      "number of user-settable variables")
        .def_readonly("dim", &simstruct::dim, "dimensionality of space.")
        .def_readonly("accuracy", &simstruct::accur,
                      "accuracy, on scale from 0 to 10")
    ...
    py::class_<Simulation>(m, "Simulation")
        ...
        //         timestep);
        .def("setSimTimes",
             [](Simulation &sim, double timestart, double timestop,
                double timestep) {
                 return smolSetSimTimes(sim.getSimPtr(), timestart, timestop,
                                        timestep);
             })

        // get/set start
        .def_property(
            "start", [](Simulation &sim) { return sim.getSimPtr()->tmin; },
        ...
        //  get/set stop
        .def_property(
            "stop", [](Simulation &sim) { return sim.getSimPtr()->tmax; },
        ...
        // get/set dt
        .def_property(
            "dt", [](Simulation &sim) { return sim.getSimPtr()->dt; },
        ...
        .def("setGraphicsParams",
             [](Simulation &sim, const char *method, int timestep, int delay) {
                 return smolSetGraphicsParams(sim.getSimPtr(), method, timestep,
                                              delay);
             })
        ...
        .def("setTiffParams",
             [](Simulation &sim, int timesteps, const char *tiffname,
                int lowcount, int highcount) {
                 return smolSetTiffParams(sim.getSimPtr(), timesteps, tiffname,
                                          lowcount, highcount);
             })
        ...
        .def("setLightParams",
             [](Simulation &sim, int lightindex, vector<double> &ambient,
                ...
                vector<double> &position) {
                 return smolSetLightParams(sim.getSimPtr(), lightindex,
                                           &ambient[0], &diffuse[0],
        ...
        .def("addOutputFile",
             [](Simulation &sim, char *filename, int suffix, bool append) {
                 return smolAddOutputFile(sim.getSimPtr(), filename, suffix,
                                          append);
             })
         ...
         ****************/
        .def("addCommand",
             [](Simulation &sim, const string &cmd, const char cmd_type,
                py::kwargs kwargs) {
        ...
        .def("getSpeciesName",
             [](Simulation &sim, int speciesindex, char *species) {
                 return smolGetSpeciesName(sim.getSimPtr(), speciesindex,
                                           species);
             })
        ...
        .def(
            ...
            [](Simulation &sim, const char *species, MolecState state,
               double difc, vector<double> &drift, vector<double> &difmatrix) {
                return smolSetSpeciesMobility(sim.getSimPtr(), species, state,
                                              difc, &drift[0], &difmatrix[0]);
            },
        ...
        .def("getMolListName",
             [](Simulation &sim, int mollistindex, char *species) {
                 return smolGetMolListName(sim.getSimPtr(), mollistindex,
                                           species);
             })
        ...
        .def("setMolList",
             [](Simulation &sim, const char *species, MolecState state,
                const char *mollist) {
                 return smolSetMolList(sim.getSimPtr(), species, state,
                                       mollist);
             })
        ...
        .def(
            ...
            [](Simulation &sim, const char *species, size_t number,
               vector<double> &lowposition, vector<double> &highposition) {
                return smolAddSolutionMolecules(sim.getSimPtr(), species,
                                                number, &lowposition[0],
                                                &highposition[0]);
            },
        ...
        .def("addCompartmentMolecules",
             [](Simulation &sim, char *species, int number,
                const char *compartment) {
                 return smolAddCompartmentMolecules(sim.getSimPtr(), species,
                                                    number, compartment);
             })
        ...
        .def("addSurfaceMolecules",
             [](Simulation &sim, const char *species, MolecState state,
                int number, const char *surface, PanelShape panelshape,
                const char *panel, vector<double> &position) {
                 return smolAddSurfaceMolecules(sim.getSimPtr(), species, state,
                                                number, surface, panelshape,
                                                panel, &position[0]);
             })
        ...
        .def("setMoleculeStyle",
             [](Simulation &sim, const char *species, MolecState state,
                double size, char *color) {
                 ...
                 return smolSetMoleculeStyle(sim.getSimPtr(), species, state,
                                             size, &rgba[0]);
             })
        .def("setMoleculeStyle",
             [](Simulation &sim, const char *species, MolecState state,
                double size, array<double, 4> &rgba) {
                 return smolSetMoleculeStyle(sim.getSimPtr(), species, state,
                                             size, &rgba[0]);
             })
        ...
        .def("setMoleculeColor",
             [](Simulation &sim, const char *species, MolecState state,
                char *color) {
                 auto rgba = color2RGBA(color);
                 return smolSetMoleculeColor(sim.getSimPtr(), species, state,
                                             &rgba[0]);
             })
        ...
        .def("setMoleculeSize",
             [](Simulation &sim, const char *species, MolecState state,
                double size) {
                 return smolSetMoleculeSize(sim.getSimPtr(), species, state,
                                            size);
             })
        ...
        .def("setBoundaryType",
             [](Simulation &sim, int dimension, int highside, char type) {
                 return smolSetBoundaryType(sim.getSimPtr(), dimension,
                                            highside, type);
             })
        ...
        .def("getSurfaceName",
             [](Simulation &sim, int surfaceindex, char *surface) {
                 return smolGetSurfaceName(sim.getSimPtr(), surfaceindex,
                                           surface);
             })
        ...
        .def(
            "setSurfaceAction",
            [](Simulation &sim, const char *surface, PanelFace face,
               const char *species, MolecState state, SrfAction action,
               const char *newspecies) {
                return smolSetSurfaceAction(sim.getSimPtr(), surface, face,
                                            species, state, action, newspecies);
            },
        ...
        .def("setSurfaceRate",
             [](Simulation &sim, const char *surface, const char *species,
                MolecState state, MolecState state1, MolecState state2,
                double rate, const char *newspecies, bool isinternal) {
                 return smolSetSurfaceRate(sim.getSimPtr(), surface, species,
                                           state, state1, state2, rate,
                                           newspecies, isinternal);
             })
        ...
        .def("addPanel",
             [](Simulation &sim, const char *surface, PanelShape panelshape,
                const char *panel, const char *axisstring,
                vector<double> &params) {
                 return smolAddPanel(sim.getSimPtr(), surface, panelshape,
                                     panel, axisstring, &params[0]);
             })
        ...
        .def("getPanelIndex",
             [](Simulation &sim, const char *surface, const char *panel) {
                 ...
                 return smolGetPanelIndex(sim.getSimPtr(), surface, &panelshape,
                                          panel);
             })
        ...
        .def("getPanelIndexNT",
             [](Simulation &sim, const char *surface, const char *panel) {
                 PanelShape panelshape{PanelShape::PSnone};
                 return smolGetPanelIndexNT(sim.getSimPtr(), surface,
                                            &panelshape, panel);
             })
        ...
        .def("getPanelName",
             [](Simulation &sim, const char *surface, PanelShape panelshape,
                int panelindex, char *panel) {
                 return smolGetPanelName(sim.getSimPtr(), surface, panelshape,
                                         panelindex, panel);
             })
        ...
        .def("setPanelJump",
             [](Simulation &sim, const char *surface, const char *panel1,
                PanelFace face1, const char *panel2, PanelFace face2,
                bool isbidirectional) {
                 return smolSetPanelJump(sim.getSimPtr(), surface, panel1,
                                         face1, panel2, face2, isbidirectional);
             })
        ...
        .def("addSurfaceUnboundedEmitter",
             [](Simulation &sim, const char *surface, PanelFace face,
                const char *species, double emitamount,
                vector<double> &emitposition) {
                 return smolAddSurfaceUnboundedEmitter(
                     sim.getSimPtr(), surface, face, species, emitamount,
                     &emitposition[0]);
             })
        ...
        .def("setSurfaceSimParams",
             [](Simulation &sim, const char *parameter, double value) {
                 return smolSetSurfaceSimParams(sim.getSimPtr(), parameter,
                                                value);
             })
        ...
        .def("addPanelNeighbor",
             [](Simulation &sim, const char *surface1, const char *panel1,
                const char *surface2, const char *panel2, int reciprocal) {
                 return smolAddPanelNeighbor(sim.getSimPtr(), surface1, panel1,
                                             surface2, panel2, reciprocal);
             })
        ...
        .def("setSurfaceStyle",
             [](Simulation &sim, const char *surface, PanelFace face,
                DrawMode mode, double thickness, char *color, int stipplefactor,
                int stipplepattern, double shininess) {
                 array<double, 4> rgba = {0, 0, 0, 1.0};
                 graphicsreadcolor(&color, &rgba[0]);
                 return smolSetSurfaceStyle(
                     sim.getSimPtr(), surface, face, mode, thickness, &rgba[0],
                     stipplefactor, stipplepattern, shininess);
             })
        .def("setSurfaceStyle",
             [](Simulation &sim, const char *surface, PanelFace face,
                DrawMode mode, double thickness, array<double, 4> &rgba,
        ...
        .def("getCompartmentName",
             [](Simulation &sim, int compartmentindex, char *compartment) {
                 return smolGetCompartmentName(sim.getSimPtr(),
        ...
        .def("addCompartmentSurface",
             [](Simulation &sim, const char *compartment, const char *surface) {
                 return smolAddCompartmentSurface(sim.getSimPtr(), compartment,
                                                  surface);
             })
        ...
        .def("addCompartmentPoint",
             [](Simulation &sim, const char *compartment,
                vector<double> &point) {
                 return smolAddCompartmentPoint(sim.getSimPtr(), compartment,
                                                &point[0]);
             })
        ...
        .def("addCompartmentLogic",
             [](Simulation &sim, const char *compartment, CmptLogic logic,
                const char *compartment2) {
                 return smolAddCompartmentLogic(sim.getSimPtr(), compartment,
                                                logic, compartment2);
             })
        ...
        .def("addReaction",
             [](Simulation &sim,
                ...
                double rate                        // rate
                 ...
                 if (nprd >= MAXPRODUCT)
                 {
                     py::print("Maximum product allowed ", MAXPRODUCT,
                               ". Given ", nprd);
                 ...
                 return smolAddReaction(
                     sim.getSimPtr(), reaction, reactant1, rstate1, reactant2,
                     rstate2, productSpecies.size(), &productSpecies[0],
        ...
        .def("getReactionIndex",
             [](Simulation &sim, vector<int> &order, const char *reaction) {
                 return smolGetReactionIndex(sim.getSimPtr(), &order[0],
                                             reaction);
             })
        ...
        .def("getReactionIndexNT",
             [](Simulation &sim, vector<int> &order, const char *reaction) {
                 return smolGetReactionIndexNT(sim.getSimPtr(), &order[0],
                                               reaction);
             })
        ...
        .def("getReactionName",
             [](Simulation &sim, int order, int reactionindex, char *reaction) {
                 return smolGetReactionName(sim.getSimPtr(), order,
        ...
        .def("setReactionRate",
             [](Simulation &sim, const char *reaction, double rate, int type) {
                 return smolSetReactionRate(sim.getSimPtr(), reaction, rate,
                                            type);
             })
        ...
        .def("setReactionRegion",
             [](Simulation &sim, const char *reaction, const char *compartment,
                const char *surface) {
                 return smolSetReactionRegion(sim.getSimPtr(), reaction,
                                              compartment, surface);
             })
        ...
        .def("setReactionProducts",
             [](Simulation &sim, const char *reaction, RevParam method,
                double parameter, const char *product,
                vector<double> &position) {
                 return smolSetReactionProducts(sim.getSimPtr(), reaction,
        ...
        .def("setReactionIntersurface",
             [](Simulation &sim, const char *reaction, vector<int> &rules) {
                 return smolSetReactionIntersurface(sim.getSimPtr(), reaction,
                                                    &rules[0]);
             })
        ...
        .def("addPort",
             [](Simulation &sim, const char *port, const char *surface,
                PanelFace face) {
        ...
        .def("addPortMolecule",
             [](Simulation &sim, const char *port, int nmolec,
                const char *species, vector<vector<double>> &pos) {
                 std::vector<double *> ptrs;
                 for (auto &vec : pos)
                     ptrs.push_back(vec.data());
                 return smolAddPortMolecules(sim.getSimPtr(), port, nmolec,
                                             species, ptrs.data());
             })
        ...
        .def("getPortMolecules",
             [](Simulation &sim, const char *port, const char *species,
                MolecState state, bool remove) {
                 return smolGetPortMolecules(sim.getSimPtr(), port, species,
                                             state, remove);
             })
        ...
        .def("addLattice",
             [](Simulation &sim, const char *lattice, const vector<double> &min,
                ...
                const char *btype) {
                 return smolAddLattice(sim.getSimPtr(), lattice, &min[0],
                                       &max[0], &dx[0], btype);
             })
        ...
        .def("addLatticeSpecies",
             [](Simulation &sim, const char *lattice, const char *species) {
                 return smolAddLatticeSpecies(sim.getSimPtr(), lattice,
                                              species);
             })
        ...
        .def("getLatticeName",
             [](Simulation &sim, int latticeindex, char *lattice) {
                 return smolGetLatticeName(sim.getSimPtr(), latticeindex,
                                           lattice);
             })
        ...
        .def("addLatticeMolecules",
             [](Simulation &sim, const char *lattice, const char *species,
                int number, vector<double> &lowposition,
                vector<double> &highposition) {
                 return smolAddLatticeMolecules(
                     sim.getSimPtr(), lattice, species, number, &lowposition[0],
                     &highposition[0]);
             })
        ...
        .def("addLatticeReaction",
             [](Simulation &sim, const char *lattice, const char *reaction,
                const int move) {
                 return smolAddLatticeReaction(sim.getSimPtr(), lattice,
                                               reaction, move);
             })
        ...
        .def("loadSimFromFile",
             [](Simulation &sim, const string &filepath, const char *flags) {
                 ...
                 return smolLoadSimFromFile(p.first.c_str(), p.second.c_str(),
                                            &psim, flags);
             })
        .def("readConfigString",
             [](Simulation &sim, const char *statement, char *params) {
                 return smolReadConfigString(sim.getSimPtr(), statement,
                                             params);
             })
    ...
    m.def("prepareSimFromFile", [](const char *filepath, const char *flags) {
        ...
        return smolPrepareSimFromFile(path.first.c_str(), path.second.c_str(),
                                      flags);
    });
