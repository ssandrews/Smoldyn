...
class Simulation(_smoldyn.Simulation):
    ...
    def run(
        ...
        **kwargs,
        ...
        k = super().run(self.stop, self.dt, display, overwrite)
    ...
    def addCommand(self, cmd: str, cmd_type: str, **kwargs):
        ...
        if "step" not in kwargs:
            kwargs["step"] = self.dt
        c = self.Command(cmd, cmd_type, simulation=super(), from_string=False, **kwargs)
    ...
    def addCommandStr(self, cmd: str):
        ...
        cmd : str
            Command string
        """
        c = self.Command(cmd, cmd_type="", simulation=super(), from_string=True)
        self.commands.append(c)
        return c
    ...
    def addSurface(self, name: str, *, panels: List[Panel]) -> Surface:
        return self.Surface(name, panels=panels)
    ...
    def addPartition(self, name: str, value):
        ...
        :py:class:`Box`
        """
        return Partition(name, value, self)
    ...
    def addMoleculePerBox(self, size: float):
        ...
        return MoleculePerBox(size, self)
    ...
    class Species(object):
        ...
        def difc(self, difconst: DiffConst):
            ...
            for state, D in self._difc.items():
                ...
                k = super().setSpeciesMobility(self.name, st, D)
                assert k == _smoldyn.ErrorCode.ok
        ...
        def mol_list(self, val):
            k = super().addMolList(val)
            assert k == _smoldyn.ErrorCode.ok, f"Failed to add mollist: {k}"
            k = super().setMolList(self.name, _toMS(self.state), val)
            assert (
        ...
        def display_size(self, size: Union[float, Dict[str, float]]):
            ...
            for state, size in self._displaySize.items():
                ...
                k = super().setMoleculeSize(self.name, state, size)
                assert k == _smoldyn.ErrorCode.ok
        ...
        def addToSolution(
            ...
            if pos:
                lowpos = highpos = pos
            k = super().addSolutionMolecules(self.name, N, lowpos, highpos)
    ...
    class Panel(object):
        def __init__(
            ...
            *,
            ...
            self.surface: Surface = self.NullSurface()
        ...
        def jumpTo(self, face1: str, panel2, face2: str, bidirectional: bool = False):
            ...
            assert isinstance(jfrom, self._PanelFace)
            jto = getattr(panel2, face2)
            assert jto, f"Could not locate {face2} on {panel2}."
            assert isinstance(jto, self._PanelFace)
        ...
        @property
        def index(self) -> int:
            v: int = super().getPanelIndexNT(self.surface.name, self.name)
            return v
    ...
    class Rectangle(Panel):
        def __init__(
            ...
            name : optional
                name of the panel.
            """
            super().__init__(simulation, shape=_smoldyn.PanelShape.rect, name=name)
            self.corner = corner
        ...
        def __init__(
            ...
            *,
            ...
            name :
                name
            """
            super().__init__(simulation, shape=_smoldyn.PanelShape.tri, name=name)
            self.axisstr = ""
    ...
    class Sphere(Panel):
        def __init__(
            ...
            *,
            ...
            """
            super().__init__(simulation, shape=_smoldyn.PanelShape.sph, name=name)
            self.center = center
            self.radius = radius
            self.slices = slices
        ...
        def __init__(
            ...
            *,
            ...
            name : optional
                name
            """
            super().__init__(simulation, shape=_smoldyn.PanelShape.hemi, name=name)
            self.center = center
            self.radius = radius
            self.vector = vector
            self.slices = slices
        ...
        def __init__(
            ...
            *,
            ...
            name : optional
                name
            """
            super().__init__(simulation, shape=_smoldyn.PanelShape.cyl, name=name)
        ...
        def __init__(
            ...
            *,
            ...
            name : optional
                name
            """
            super().__init__(simulation, shape=_smoldyn.PanelShape.disk, name=name)
            self.center = center
            self.radius = radius
            self.vector = vector
            # Smoldyn panel
            ...
            *points:
            ...
            for p1, p2 in zip(self.points, self.points[1:]):
                ...
                if theta in [0.0, math.pi, math.pi / 2.0, -math.pi / 2.0]:
                    ...
                    p = Rectangle(
                        super(), corner=(x1, y1), dimensions=[length], axis=axis
                    )
                    __logger__.info(f"Added a Rectangle {p}")
                    self.panels.append(p)
                else:
                    t = Triangle(super(), vertices=((x1, y1), (x2, y2)))
...
# TODO: Add a Path3D or Surface3D. not sure what to call it.
    ...
    class Surface(object):
        ...
        def __init__(self, name: str, *, panels: List[Panel]):
            ...
            self.panels = panels
            self.name = name
            super().addSurface(self.name)

            self.front = self._SurfaceFaceCollection(self.smoldyn, ["front"], name)
            self.back = self._SurfaceFaceCollection(self.smoldyn, ["back"], name)
            self.both = self._SurfaceFaceCollection(self.smoldyn, ["front", "back"], name)
            self._addToSmoldyn()
    ...
    class Port(object):
        ...
        def __init__(self, name: str, surface: Union[Simulation.Surface, str], panel: str):
            """
            Parameters
            ----------
            name : str
            ...
            """
            self.name: str = name
            self.surfname = surface.name if isinstance(surface, Simulation.Surface) else surface
            assert panel in ["front", "back"]
            self.panel = _smoldyn.PanelFace.__members__[panel]
            k = super().addPort(self.name, self.surfname, self.panel)
            assert k == _smoldyn.ErrorCode.ok
    ...
    class Partition:
        ...
        def __post_init__(self):
            ...
            :py:class:`Box`
            """
            k = self.sim.setPartitions(self.name, self.value)
            assert k == _smoldyn.ErrorCode.ok, f"Failed to set partition: {k}"


    class MoleculePerBox(Simulation.Partition):
    ...
    class Compartment(object):
        def __init__(
            ...
            name: str,
            *,
            surface: Union[str, Simulation.Surface],
            point: List[float]
        ):
            """Comapartment.
            ...
            """
            self.name = name
            self.srfname = surface.name if isinstance(surface, Simulation.Surface) else surface
            self.point = point

            assert self.point, "At least one point is required."
            assert super(), "Simulation() is missing."
            k = super().addCompartment(self.name)
            assert k == _smoldyn.ErrorCode.ok
            k = super().addCompartmentSurface(self.name, self.srfname)
            assert k == _smoldyn.ErrorCode.ok
            k = super().addCompartmentPoint(self.name, self.point)
            assert k == _smoldyn.ErrorCode.ok

        def addMolecules(self, species: Simulation.Species, N: int):
            ...
            k = super().addCompartmentMolecules(species.name, N, self.name)
            assert k == _smoldyn.ErrorCode.ok


    class Command(object):
        def __init__(self, cmd: str, cmd_type, **kwargs):
            """Smoldyn Command.
    ...
    class Reaction(object):
        ...
        def __init__(
            self,
            name: str,
            subs: List[Simulation.SpeciesWithState],
            prds: List[Simulation.SpeciesWithState],
            *,
            rate: float = 0.0,
            compartment: Simulation.Compartment = None,
            surface: Simulation.Surface = None,
            binding_radius: float = 0.0,
            reaction_probability: float = 0.0,
        ):
            ...
            if subs is None or len(subs) == 0:
                ...
                subs = [Simulation.NullSpecies()]
            r1 = subs[0]
            r2 = subs[1] if len(subs) == 2 else Simulation.NullSpecies()

            if isinstance(r1, Simulation.Species):
            ...
            if isinstance(r2, Simulation.Species):
            ...
            prdNames, prdStates = [], []
            for x in prds:
                if isinstance(x, Simulation.Species):
            ...
            if self.compartment is not None or self.surface is not None:
                ...
                k = super().setReactionRegion(self.name, cname, sname)
                assert k == _smoldyn.ErrorCode.ok
        ...
        def setRate(self, rate, reaction_probability=0.0, binding_radius=0.0):
            ...
            if rate >= 0.0:
                k = super().setReactionRate(self.name, rate, False)
                assert k == _smoldyn.ErrorCode.ok
                return
            elif rate < 0.0:
        ...
        def setIntersurface(self, rules: List[Union[int, str]]):
            ...
            for r in rules:
                ...
                _rules.append(r)
            k = super().setReactionIntersurface(self.name, _rules)
            assert k == _smoldyn.ErrorCode.ok
        ...
        def productPlacement(
            ...
            k = super().setReactionProducts(self.name, revType, param, product, pos)
            assert k == _smoldyn.ErrorCode.ok
    ...
    class BidirectionalReaction(object):
        def __init__(
            ...
            *,
            ...
            self._kf = kf
            self._kb = kb
            self.forward = Simulation.Reaction(
                fwdname,
                subs,
                prds,
                rate=kf,
            ...
            self.reverse = None
            if self._kb > 0.0:
                self.reverse = Simulation.Reaction(
                    revname,
                    prds,
                    subs,
                    rate=self._kb,
