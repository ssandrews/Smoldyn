...
void writesim(simptr sim, FILE *fptr)
{
    ...
    fprintf(
        fptr, "random_seed %li  # this is a new random number\n", (long int)randULI());
...
int simreadstring(simptr sim, ParseFilePtr pfp, const char *word, char *line2)
{
    ...
    // space and time
        ...
        itct =
            strmathsscanf(line2, "%s %mlg %c", varnames, varvalues, nvar, nm, &flt1, &ch);
    ...
    // molecules
        ...
        er = molstring2index1(sim, line2, &ms, &index);
        CHECKS(
            er != -1, "surface_drift format: species(state) surface panel_shape values");
        CHECKS(er != -2, "mismatched or improper parentheses around molecule state");
        CHECKS(er != -3, "cannot read molecule state value");
        CHECKS(er != -4, "molecule name not recognized");
        CHECKS(er != -6, "cannot read wildcard logic");
        ...
        CHECKS(
            itct == 2, "surface_drift format: species(state) surface panel_shape values");
        if(!strcmp(nm, "all"))
            s = -1;
        else {
        ...
        CHECKS(itct == 1,
            ...
            "panel_name [position]");
        CHECKS(
            nmol >= 0, "in surface_mol, the number of molecules needs to be at least 0");
        CHECKS(line2 = strnword(line2, 2),
            "surface_mol format: nmol species(state) surface panel_shape "
            "panel_name [position]");
        i = molstring2index1(sim, line2, &ms, NULL);
    ...
    // about runtime commands
        ...
        er = scmdstr2cmd(
            sim->cmds, line2, sim->tmin, sim->tmax, sim->dt, varnames, varvalues, nvar);
    ...
    // compartments
        ...
        itct = sscanf(line2, "%s %s", nm, nm1);
        CHECKS(
            itct == 2, "compartment format: compart_name statement_name statement_text");
        line2 = strnword(line2, 3);
    ...
    // filaments
        ...
        CHECKS(
            sim->filss, "individual filaments need to be defined before using filament");
        itct = sscanf(line2, "%s %s", nm, nm1);
    ...
    // reactions
        ...
        if(r >= 0) {
            CHECKS(
                order == 2, "reaction chi value is only allowed for order 2 reactions");
        ...
        if(rpart == RPirrev)
            ...
            flt1 = -2;
                ...
                rpart == RPpgemmaxw || rpart == RPratio || rpart == RPunbindrad ||
                rpart == RPpgem2 || rpart == RPpgemmax2 || rpart == RPratio2) {
            ...
            if(strbegin("product_", nm1, 0)) {
                ...
                CHECKS(
                    i != -2, "mismatched or improper parentheses around product state");
                CHECKS(i != -3, "cannot read molecule state value");
                CHECKS(i != -4, "molecule name not recognized");
...
int simInitAndLoad(const char *fileroot, const char *filename, simptr *smptr,
    const char *flags, ValueProviderFactory *valueProviderFactory, AbstractMesh *mesh)
{
...
/* simInitAndLoad */
int simInitAndLoad(
    const char *fileroot, const char *filename, simptr *smptr, const char *flags)
{
    simptr sim;
    int er, qflag;
...
void debugcode(simptr sim, const char *prefix)
{
    ...
    for(m = 0; m < sim->mols->nl[0]; m++) {
        ...
        if(mptr->serno == 1377166 || mptr->serno == 1374858) {
            ...
            printf(" posx side=%s",
                surfface2string(
                    panelside(mptr->posx, sim->srfss->srflist[4]->panels[PSdisk][0], 3,
                        NULL, 1, 0),
                    string));
            printf(" pos side=%s",
                surfface2string(
                    panelside(mptr->pos, sim->srfss->srflist[4]->panels[PSdisk][0], 3,
                        NULL, 1, 0),
                    string));
            printf("\n");
        }
...
void endsimulate(simptr sim, int er)
{
    ...
    if(er == 1)
        ...
        simLog(
            sim, 5, "Simulation terminated during molecule sorting\n  Out of memory\n");
    else if(er == 7)
