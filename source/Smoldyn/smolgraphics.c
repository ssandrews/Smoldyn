...
// enumerated types
char*
graphicslp2string(enum LightParam lp, char* string);
...
int
graphicsupdateinit(simptr sim);
int
graphicsupdatelists(simptr sim);
int
graphicsupdateparams(simptr sim);

// core simulation functions
void
RenderSurfaces(simptr sim);
void
RenderMolecs(simptr sim);
void
RenderText(simptr sim);
void
RenderSim(simptr sim, int swapbuffers);
...
/* graphicsreadcolor */
int
graphicsreadcolor(char** stringptr, double* rgba)
{
...
/* graphssfree */
void
graphssfree(graphicsssptr graphss)
{
    int item;
...
graphssoutput(simptr sim)
{
    ...
    if (graphss->framepts)
        simLog(sim,
          ...
          graphss->framecolor[1],
          graphss->framecolor[2],
    ...
    for (lt = 0; lt < MAXLIGHTS; lt++)
        if (graphss->lightstate[lt] != LPauto) {
            ...
            simLog(sim,
              2,
              "  position: %g|L %g|L %g|L %g\n",
              graphss->lightpos[lt][0],
              graphss->lightpos[lt][1],
            ...
            simLog(sim,
              ...
              graphss->ambilight[lt][1],
              graphss->ambilight[lt][2],
              graphss->ambilight[lt][3]);
            simLog(sim,
              2,
              "  diffuse: %g %g %g %g\n",
              graphss->difflight[lt][0],
              graphss->difflight[lt][1],
              graphss->difflight[lt][2],
              graphss->difflight[lt][3]);
            simLog(sim,
              2,
              "  specular: %g %g %g %g\n",
              graphss->speclight[lt][0],
              graphss->speclight[lt][1],
              graphss->speclight[lt][2],
...
/* writegraphss */
void
writegraphss(simptr sim, FILE* fptr)
{
    graphicsssptr graphss;
    char string[STRCHAR];
    int lt, item;
    ...
    for (lt = 0; lt < MAXLIGHTS; lt++)
        if (graphss->lightstate[lt] != LPauto) {
            fprintf(fptr,
              "light %i position %g %g %g\n",
              lt,
              graphss->lightpos[lt][0],
              graphss->lightpos[lt][1],
              graphss->lightpos[lt][2]);
            fprintf(fptr,
              "light %i ambient %g %g %g %g\n",
              lt,
              graphss->ambilight[lt][0],
              graphss->ambilight[lt][1],
              graphss->ambilight[lt][2],
...
/* checkgraphicsparams */
int
checkgraphicsparams(simptr sim, int* warnptr)
{
    int error, warn;
    graphicsssptr graphss;
    char string[STRCHAR];
...
void
graphicssetcondition(graphicsssptr graphss, enum StructCond cond, int upgrade)
{
    if (!graphss)
        return;
...
int
graphicsenablegraphics(simptr sim, const char* type)
{
    graphicsssptr graphss;
    int code;
...
/* graphicssetiter */
int
graphicssetiter(simptr sim, int iter)
{
    int er;
...
/* graphicssettiffiter */
int
graphicssettiffiter(simptr sim, int iter)
{
    int er;
...
/* graphicssetdelay */
int
graphicssetdelay(simptr sim, int delay)
{
    int er;
...
int
graphicssetframethickness(simptr sim, double thickness)
{
    int er;
...
int
graphicssetframecolor(simptr sim, double* color)
{
    int er;
...
int
graphicssetgridthickness(simptr sim, double thickness)
{
    int er;
...
int
graphicssetgridcolor(simptr sim, double* color)
{
    int er;
...
int
graphicssetbackcolor(simptr sim, double* color)
{
    int er;
...
int
graphicssettextcolor(simptr sim, double* color)
{
    int er;
...
/* graphicssettextitem */
int
graphicssettextitem(simptr sim, char* itemname)
{
    graphicsssptr graphss;
        ...
        for (s = 0; s < srfss->nsrf; s++) {
            ...
            while (fdm || bdm) {
                ...
                for (p = 0; p < srf->npanel[PSdisk]; p++) { // 3-D disks
                    ...
                    gl2DrawCircleD(
                      vect2, point[1][0], (int)point[1][1], 'f', 3); //?? 'f' isn't right
                    glPopMatrix();
                }
                    ...
                    for (f = 0; f < filtype->nfil; f++) {
                        ...
                        for (vtx = 0; vtx < fil->nseg; vtx++) {
                            ...
                            glRotated((GLdouble)theta,
                              ...
                              (GLdouble)(axis[1]),
                              (GLdouble)(axis[2]));
                            gl2DrawCylinder(filtype->edgepts,
                              filtype->edgepts,
                              fil->segments[vtx]->len,
                              8,
                              2,
                              0,
                              graphics >= 3 ? 1 : 0);
...
RenderLattice(simptr sim)
{
    ...
    for (lat = 0; lat < sim->latticess->nlattice; lat++) {
        ...
        for (ilat = 0; ilat < lattice->nspecies; ilat++) { // ilat is the species identity
            ...
            for (i = 0; i < n;
              ++i) { // n is the total number of molecules of this species, copy_numbers
                     // is how many in each site, and positions are the site positions
...
RenderScene(void);
void
TimerFunction(int state);

simptr Sim;

/* smolPostRedisplay */
void
smolPostRedisplay(void)
{
#ifdef __gl_h_
    glutPostRedisplay();
#endif
