...
int srfsamestate(enum MolecState ms1, enum PanelFace face1, enum MolecState ms2,
    enum MolecState *ms3ptr);
void srfreverseaction(enum MolecState ms1, enum PanelFace face1, enum MolecState ms2,
    enum MolecState *ms3ptr, enum PanelFace *face2ptr, enum MolecState *ms4ptr);
void srftristate2index(enum MolecState ms, enum MolecState ms1, enum MolecState ms2,
    enum MolecState *ms3ptr, enum PanelFace *faceptr, enum MolecState *ms4ptr);

// memory management
surfactionptr surfaceactionalloc(int species);
void surfaceactionfree(surfactionptr actdetails);
int panelsalloc(
    surfaceptr srf, int dim, int maxpanel, int maxspecies, enum PanelShape ps);
...
double srfcalcrate(simptr sim, surfaceptr srf, int i, enum MolecState ms1,
    enum PanelFace face, enum MolecState ms2);
double srfcalcprob(simptr sim, surfaceptr srf, int i, enum MolecState ms1,
    enum PanelFace face, enum MolecState ms2);
...
void surfacereflect(
    moleculeptr mptr, panelptr pnl, double *crsspt, int dim, enum PanelFace face);
int surfacejump(
    moleculeptr mptr, panelptr pnl, double *crsspt, enum PanelFace face, int dim);
int dosurfinteract(simptr sim, moleculeptr mptr, int ll, int m, panelptr pnl,
    enum PanelFace face, double *crsspt);
...
int readsurfacename(simptr sim, const char *str, enum PanelShape *psptr, int *pptr)
{
    ...
    if(!strcmp(snm, "all")) {  // all surfaces
        ...
        if(s == -1)  // unknown surface
            ...
            if(pnm[0] == '\0')
            ...
            else if(VCellDefined && strstr(pnm, "tri_") == pnm) {  // surface:tri_#_#_#
                                                                   // (VCELL)
                ps = PStri;
                int memIndex;
                int globalIndex;
                ...
                for(ps = (PanelShape)0; p == -1 && ps < PSMAX; ps = (PanelShape)(ps + 1))
                    p = stringfind(sim->srfss->srflist[s]->pname[ps],
                        sim->srfss->srflist[s]->npanel[ps], pnm);
                if(p == -1) {
                    ps = PSnone;
                    p  = -3;
                }
...
/* surfacearea2 */
double surfacearea2(
    simptr sim, int surface, enum PanelShape ps, char *pname, int *totpanelptr)
{
    int panel, totpanel;
    double area;
    surfaceptr srf;
    ...
    else if(surface >= 0 && ps != PSall && pname && strcmp(pname, "all")) {  // specific
                                                                             // panel
...
/* srfindex2tristate */
void srfindex2tristate(enum MolecState ms3, enum PanelFace face, enum MolecState ms4,
    enum MolecState *msptr, enum MolecState *ms1ptr, enum MolecState *ms2ptr)
{
...
/* srfcompareaction */
int srfcompareaction(enum SrfAction act1, surfactionptr details1, enum SrfAction act2,
...
int emittersalloc(surfaceptr srf, enum PanelFace face, int oldmaxspecies, int maxspecies)
{
    ...
    for(ps = (PanelShape)0; ps < PSMAX;
        ps = (PanelShape)(ps + 1))  // panel structure data
...
int surfexpandmollist(surfaceptr srf, int newmax, int ll)
{
    ...
    for(m = 0; m < srf->nmol[ll] && m < newmax; m++)
...
surfaceptr surfacealloc(surfaceptr srf, int oldmaxspecies, int maxspecies, int dim)
{
    ...
    if(maxspecies) {
        CHECKMEM(newaction =
                     (enum SrfAction ***)calloc(maxspecies, sizeof(enum SrfAction **)));
        for(i = 0; i < maxspecies; i++)
            newaction[i] = NULL;
        for(i = 0; i < oldmaxspecies; i++)
            newaction[i] = srf->action[i];
        for(; i < maxspecies; i++) {
            CHECKMEM(newaction[i] =
                         (enum SrfAction **)calloc(MSMAX, sizeof(enum SrfAction *)));
            for(ms = (MolecState)0; ms < MSMAX; ms = (MolecState)(ms + 1))
                newaction[i][ms] = NULL;
            for(ms = (MolecState)0; ms < MSMAX; ms = (MolecState)(ms + 1)) {
                CHECKMEM(newaction[i][ms] =
                             (enum SrfAction *)calloc(3, sizeof(enum SrfAction)));
                newaction[i][ms][PFfront] = newaction[i][ms][PFback] = SAtrans;
                newaction[i][ms][PFnone]                             = SAno;
            }
        }

        CHECKMEM(newactdetails =
                     (surfactionptr ***)calloc(maxspecies, sizeof(surfactionptr **)));
        for(i = 0; i < maxspecies; i++)
            newactdetails[i] = NULL;
        for(i = 0; i < oldmaxspecies; i++)
            newactdetails[i] = srf->actdetails[i];
        for(; i < maxspecies; i++) {
            CHECKMEM(newactdetails[i] =
                         (surfactionptr **)calloc(MSMAX, sizeof(surfactionptr *)));
            for(ms = (MolecState)0; ms < MSMAX; ms = (MolecState)(ms + 1))
...
surfacessptr surfacessalloc(surfacessptr srfss, int maxsurface, int maxspecies, int dim)
{
    ...
    if(maxsurface > srfss->maxsrf) {  // allocate any new surface names and surfaces
        ...
        CHECKMEM(
            newnames = (char **)calloc(maxsurface, sizeof(char *)));  // surface names
        ...
        CHECKMEM(newsrflist = (surfaceptr *)calloc(
                     maxsurface, sizeof(surfaceptr)));  // surface list
        for(s = 0; s < maxsurface; s++)
            newsrflist[s] = NULL;
        for(s = 0; s < oldmaxsrf; s++)
...
void surfaceoutput(simptr sim)
{
    ...
    for(s = 0; s < srfss->nsrf; s++) {
        ...
        if(srf->port[PFfront])
            simLog(sim, 2, "  The front of this surface is part of port %s\n",
                srf->port[PFfront]->portname);
        if(srf->port[PFback])
            simLog(sim, 2, "  The back of this surface is part of port %s\n",
        ...
        if(sim->mols && srf->action) {
            ...
            for(i = 1; i < nspecies; i++) {
                for(face = (PanelFace)0; face < 2; face = (PanelFace)(face + 1)) {
                    ...
                    if(same) {
                        if(act != SAmult) {
                            simLog(sim, 2, "   %s(all) at %s: %s", sim->mols->spname[i],
                                face == PFfront ? "front" : "back",
                                surfact2string(act, string));
                            if(newident)
                                simLog(sim, 2, " (convert to %s)",
                        ...
                        for(ms = (MolecState)0; ms < MSMAX; ms = (MolecState)(ms + 1)) {
                            ...
                            if(sim->mols->exist[i][ms] && act != SAmult) {
                                simLog(sim, 2, "   %s(%s)", sim->mols->spname[i],
                            ...
                            for(ms2 = (MolecState)0; ms2 < MSMAX1;
                                ...
                                if(prob > 0 && !srfsamestate(ms, face, ms2, NULL)) {
                                    ...
                                    if(actdetails->srfnewspec[ms2] != i)
                                        simLog(sim, 2, " (convert to %s)",
                                            sim->mols
                                                ->spname[actdetails->srfnewspec[ms2]]);
                                    if(actdetails->srfdatasrc[ms2] == 1)
                                        simLog(sim, 2, ", requested rate=%g",
                                            actdetails->srfrate[ms2]);
                                    else if(actdetails->srfdatasrc[ms2] == 2)
                                        simLog(sim, 2, ", requested prob=%g",
        ...
        simLog(sim, 2, "  front color: %g %g %g %g\n", srf->fcolor[0], srf->fcolor[1],
            srf->fcolor[2], srf->fcolor[3]);
        simLog(sim, 2, "  back color: %g %g %g %g\n", srf->bcolor[0], srf->bcolor[1],
            srf->bcolor[2], srf->bcolor[3]);
        simLog(sim, 2, "  edge points: %g, polygon modes: %s %s\n", srf->edgepts,
            surfdm2string(srf->fdrawmode, string), surfdm2string(srf->bdrawmode, string));
        if(srf->edgestipple[1] != 0xFFFF)
            simLog(sim, 2, "   edge stippling: %ui %X\n", srf->edgestipple[0],
                srf->edgestipple[1]);
        if(srf->fshiny != 0)
                ...
                for(i = 0; i < nspecies; i++)
                    if(srf->nemitter[face][i]) {
                        simLog(sim, 2,
                            ...
                            srf->nemitter[face][i], surfface2string(face, string),
        ...
        if(srf->maxpanel[PSrect]) {
            ...
            for(p = 0; p < srf->npanel[PSrect] && p < 21; p++) {
                ...
                front = pnl->front;
                if(dim == 1)
                    simLog(sim, 2, "   %s: %g, facing %c0", pname[p], point[0][0],
                        front[0] == 1 ? '+' : '-');
                else if(dim == 2)
                    simLog(sim, 2, "   %s: (%g,%g), (%g,%g), facing: %c%1.0f, length: %g",
                        pname[p], point[0][0], point[0][1], point[1][0], point[1][1],
                        front[0] == 1 ? '+' : '-', front[1],
                ...
                simLog(sim, 2, "\n");
                if(pnl->maxneigh) {
                    simLog(sim, 1, "    neighbors allocated: %i, defined: %i",
                        pnl->maxneigh, pnl->nneigh);
        ...
        if(srf->maxpanel[PStri]) {
            ...
            for(p = 0; p < srf->npanel[PStri] && p < 21; p++) {
                ...
                if(dim == 1)
                    ...
                    simLog(sim, 2, "   %s: (%g,%g), (%g,%g), facing: (%g,%g), length: %g",
                        pname[p], point[0][0], point[0][1], point[1][0], point[1][1],
                        front[0], front[1], Geo_LineLength(point[0], point[1], 2));
                else
                    simLog(sim, 2,
        ...
        if(srf->maxpanel[PSsph]) {
            ...
            for(p = 0; p < srf->npanel[PSsph] && p < 21; p++) {
                ...
                front = pnl->front;
                if(dim == 1)
                    simLog(sim, 2, "   %s: %g, R=%g, facing: %s", pname[p], point[0][0],
                        point[1][0], front[0] == 1 ? "out" : "in");
                else if(dim == 2)
                    simLog(sim, 2,
                        "   %s: (%g,%g), R=%g, facing: %s, draw: %g, length: %g",
                        pname[p], point[0][0], point[0][1], point[1][0],
                        front[0] == 1 ? "out" : "in", point[1][1],
                        2.0 * PI * point[1][0]);
                else
                    simLog(sim, 2,
                        "   %s: (%g,%g,%g), R=%g, facing: %s, draw: %g %g, area: %g",
                        pname[p], point[0][0], point[0][1], point[0][2], point[1][0],
                        front[0] == 1 ? "out" : "in", point[1][1], point[1][2],
            ...
            for(p = 0; p < srf->npanel[PScyl] && p < 21; p++) {
                ...
                if(dim == 1)
                    ...
                    simLog(sim, 2,
                        ...
                        "area: %g",
                        pname[p], point[0][0], point[0][1], point[0][2], point[1][0],
                        point[1][1], point[1][2], point[2][0],
                        front[2] == 1 ? "out" : "in", point[2][1], point[2][2],
            ...
            for(p = 0; p < srf->npanel[PShemi] && p < 21; p++) {
                ...
                if(dim == 1)
                    ...
                    simLog(sim, 2,
                        ...
                        "length: %g",
                        pname[p], point[0][0], point[0][1], point[1][0],
                        front[0] == 1 ? "out" : "in", point[2][0], point[2][1],
                        point[1][1], PI * point[1][0]);
                else
                    simLog(sim, 2,
        ...
        if(srf->maxpanel[PSdisk]) {
            ...
            for(p = 0; p < srf->npanel[PSdisk] && p < 21; p++) {
                ...
                if(dim == 1)
                    ...
                    simLog(sim, 2, "   %s: (%g,%g), R=%g, facing: (%g,%g), length: %g",
                        pname[p], point[0][0], point[0][1], point[1][0], front[0],
                        front[1], 2.0 * point[1][0]);
                else
                    simLog(sim, 2,
                        "   %s: (%g,%g,%g), R=%g, facing: (%g,%g,%g), draw: %g, area: %g",
                        pname[p], point[0][0], point[0][1], point[0][2], point[1][0],
                        front[0], front[1], front[2], point[1][1],
                ...
                for(ms1 = (MolecState)0; ms1 < MSMAX; ms1 = (MolecState)(ms1 + 1))
                    for(face = (PanelFace)0; face < 2; face = (PanelFace)(face + 1)) {
                        ...
                        if(act != SAtrans && act != SAmult) {
                            fprintf(fptr, "action %s(%s)", sim->mols->spname[i],
                                molms2string(ms1, string));
                            fprintf(fptr, " %s %s", face == PFfront ? "front" : "back",
                                surfact2string(act, string));
                        ...
                        if(srf->action[i][ms1][face] == SAmult &&
                            ...
                            for(ms2 = (MolecState)0; ms2 < MSMAX1;
                                ...
                                if(actdetails->srfrate[ms2] > 0) {
                                    fprintf(fptr, "rate %s(%s)", sim->mols->spname[i],
                                        molms2string(ms1, string));
                                    fprintf(fptr, " %s",
                                        (face == PFnone)
                                            ? molms2string(ms1, string)
                                            : (face == PFfront ? "fsoln" : "bsoln"));
                                    fprintf(fptr, " %s %g", molms2string(ms2, string),
                                        actdetails->srfrate[ms2]);
                                    if(actdetails->srfnewspec[ms2] != i)
                                        fprintf(fptr, " %s",
                                            sim->mols
                                    ...
                                    fprintf(fptr, "rate_internal %s(%s)",
                                        sim->mols->spname[i], molms2string(ms1, string));
                                    fprintf(fptr, " %s",
                                        (face == PFnone)
                                            ? molms2string(ms1, string)
                                            : (face == PFfront ? "fsoln" : "bsoln"));
        ...
        for(ps = (PanelShape)0; ps < PSMAX; ps = (PanelShape)(ps + 1)) {
            ...
            for(p = 0; p < srf->npanel[ps]; p++) {
                ...
                if(ps == PSrect) {
                    ...
                    fprintf(fptr, " %c%g", front[dim == 2 ? 2 : 0] > 0 ? '+' : '-',
                        point[2][0]);
                    if(dim > 2)
                ...
                if(pnl->nneigh) {
                    ...
                    for(j = 0; j < pnl->nneigh; j++) {
                        if(pnl->neigh[j]->srf == srf)
                            ...
                            fprintf(fptr, " %s:%s", pnl->neigh[j]->srf->sname,
                                pnl->neigh[j]->pname);
                    }
                    fprintf(fptr, "\n");
                }
...
int checksurfaceparams(simptr sim, int *warnptr)
{
    ...
    if(sim->mols)  // nmollist
        if(srfss->nmollist != sim->mols->nlist) {
            error++;
            simLog(sim, 10,
                " SMOLDYN BUG: mismatch between number of molecule lists in surface and "
    ...
    if(srfss->neighdist > 0.01 * syslen) {
        warn++;
        simLog(sim, 5,
            " WARNING: surface neighbor distance value is large compared to system "
            "size\n");
    }
    ...
    for(s = 0; s < srfss->nsrf;
        ...
        if(srf->fshiny < 0 || srf->fshiny > 128) {
            error++;
            simLog(sim, 10, " SMOLDYN BUG: surface %i front shininess is out of bounds\n",
                s);
        }
        ...
        for(ps = (PanelShape)0; ps < PSMAX; ps = (PanelShape)(ps + 1)) {
            ...
            if(srf->npanel[ps] > 0) {
                if(!srf->pname[ps]) {
                    error++;
                    simLog(sim, 10,
                        " SMOLDYN BUG: surface %i has no name strings allocated for "
                        "shape %i\n",
                        s, ps);
                }
                if(!srf->panels[ps]) {
                    error++;
                    simLog(sim, 10,
                        " SMOLDYN BUG: surface %i has panels listed but not allocated "
                        "for shape %i\n",
                        s, ps);
                }
            }
            ...
            for(p = 0; p < srf->npanel[ps]; p++) {
                ...
                if(ps == PSrect) {
                    if(dim == 1) {
                        ...
                        if(!Geo_LineXaabb2(point[0], point[1], norm, lowwall, highwall)) {
                            warn++;
                            simLog(sim, 5,
                                " WARNING: surface %s, panel %s is entirely outside the "
                                "system volume\n",
                ...
                for(ps = (PanelShape)0; ps < PSMAX; ps = (PanelShape)(ps + 1))
                    for(p = 0; p < srf->npanel[ps]; p++) {
                        ...
                        if(!pnl->jumpp[PFfront]) {
                            ...
                            warn++;
                            simLog(sim, 5,
                                " WARNING: surface %s, panel %s front is a jump type "
                                "panel that is its own destination\n",
                                srf->sname, pnl->pname);
                        }
                        if(pnl->jumpp[PFfront] && !(pnl->jumpf[PFfront] == PFfront ||
                            ...
                            error++;
                            simLog(sim, 10,
                                " SMOLDYN BUG: surface %s, panel %s jumps to an "
                                "undefined panel face\n",
                                srf->sname, pnl->pname);
                        }
                    }
            if(bjump)
                for(ps = (PanelShape)0; ps < PSMAX; ps = (PanelShape)(ps + 1))
                    for(p = 0; p < srf->npanel[ps]; p++) {
                        ...
                        if(!pnl->jumpp[PFback]) {
                            warn++;
                            simLog(sim, 5,
                                " WARNING: back of surface %s has jump action but panel "
                                "%s has no jump destination\n",
                                srf->sname, pnl->pname);
                        }
                            ...
                            warn++;
                            simLog(sim, 5,
                                " WARNING: surface %s, panel %s back is a jump type "
                                "panel that is its own destination\n",
                                srf->sname, pnl->pname);
                        }
    ...
    for(s = 0; s < srfss->nsrf; s++) {  // make sure porting actions are linked to ports
        ...
        if(srf->action) {
            ...
            if(bport && !srf->port[PFback]) {
                error++;
                simLog(sim, 9,
                    " ERROR: surface %s back face has porting action, but is not linked "
                    "to a port\n",
                    srf->sname);
                for(i = 0; i < nspecies; i++)
                    if(srf->action[i][MSsoln][PFback] == SAport)
                    ...
                    for(ms = (MolecState)0; ms < MSMAX; ms = (MolecState)(ms + 1))
                        for(face = (PanelFace)0; face < 3; face = (PanelFace)(face + 1))
                            if(srf->action[i][ms][face] == SAmult) {
                                ...
                                if(!actdetails) {
                                    simLog(sim, 10,
                                        " SMOLDYN BUG: action is SAmult but action "
                                        "details not allocated");
                                    error++;
                                }
                                else {
                                    for(ms2 = (MolecState)0; ms2 < MSMAX1;
                                        ...
                                        if(prob < 0 || prob > 1) {
                                            ...
                                            if(actdetails->srfdatasrc[ms2] == 1)
                                                if(fabs((actdetails->srfrate[ms2] -
                                                            srfcalcrate(sim, srf, i, ms,
                                                                face, ms2)) /
                                                        actdetails->srfrate[ms2]) >
                                                    0.01) {
                                                    warn++;
                                                    simLog(sim, 5,
                                                        " WARNING: requested surface "
                                                        "'%s' rate for %s(%s) -> %s(%s) "
                                                        "cannot be achieved\n",
                                                        srf->sname, sim->mols->spname[i],
                                                        molms2string(ms, string),
                                                        sim->mols->spname[i],
                        ...
                        if(srf->nemitter[face] && srf->nemitter[face][i])
                            if(srf->action[i][MSsoln][face] != SAreflect) {
                                ...
                                simLog(sim, 5,
                                    ...
                                    srf->sname, surfface2string(face, string),
                                    sim->mols->spname[i],
                                    surfact2string(
                                        srf->action[i][MSsoln][face], string2));
                            }
                }
            }
        }
                            ...
                            for(p = 0; p < srf->npanel[ps]; p++)
                                if(srf->panels[ps][p]->emitterabsorb[face][i] == 1.0) {
                                    ...
                                    simLog(sim, 5,
                                        ...
                                        "diffusion\n",
                                        srf->sname, srf->panels[ps][p]->pname,
                                        surfface2string(face, string),
                                        sim->mols->spname[i]);
                                }
        }
    ...
    if(sim->srfss)  // check for redundant function call
        if(maxsurf == -1 || sim->srfss->maxsrf == maxsurf)
            if((sim->mols && sim->srfss->maxspecies == sim->mols->maxspecies) ||
                ...
                return 0;
    srfss = surfacessalloc(sim->srfss, maxsurf < 0 ? 5 : maxsurf,
        sim->mols ? sim->mols->maxspecies : 0, sim->dim);
    if(!srfss)
        return 1;
    sim->srfss = srfss;
    srfss->sim = sim;
...
/* surfsetaction */
int surfsetaction(surfaceptr srf, int ident, const int *index, enum MolecState ms,
            ...
            for(i = 1; i < sim->mols->nspecies; i++)
                for(ms1 = mslo; ms1 <= mshi; ms1 = MolecState(ms1 + 1)) {
                    if(face == PFfront || face == PFboth) {
                        ...
                        if(!actdetails) {
                            actdetails =
                                surfaceactionalloc(i);  // allocate action details
                            srf->actdetails[i][ms1][PFfront] = actdetails;
                        }
                        if(!actdetails)
                            return -1;
...
int surfsetrate(surfaceptr srf, int ident, const int *index, enum MolecState ms,
    enum MolecState ms1, enum MolecState ms2, int newident, double value, int which)
{
    int i, j;
    enum MolecState ms3, ms4;
    enum PanelFace face;
...
#ifdef OPTION_VCELL
/* surfSetRateExp */
int surfSetRateExp(surfaceptr srf, int ident, enum MolecState ms, enum MolecState ms1,
    enum MolecState ms2, int newident, ValueProvider *valueProvider, int which)
{
    int i1, ilo, ihi;
    enum MolecState ms3, ms4;
    enum PanelFace face;
...
/* surfaddpanel */
int surfaddpanel(surfaceptr srf, int dim, enum PanelShape ps, const char *string,
    double *params, const char *name)
{
    ...
    if(ps == PSrect) {  // rectangle
        ...
        if(dim == 1) {
        ...
        else if(dim == 2) {  // params[0] and [1] are the given starting point, [2] is the
                             // length
            pt = 1;
            if(ch == '-')
                pt *= -1;
            ...
            front[1]         = (double)pdim;
            if(pdim == 0)
                axis01 = (pt == 1 ? 1 : 2);  // axis01 is the axis parallel to edge from
                                             // point 0 to point 1
            else if(pdim == 1)
                axis01 = (pt == 1 ? 2 : 0);
            else
...
/* surftransformpanel */
void surftransformpanel(
    panelptr pnl, int dim, double *translate, double *origin, double *expand)
{
    int isotropic;
                    ...
                    if(srf->nemitter[face][i]) {
                        ...
                        for(ps = (PanelShape)0; ps < PSMAX; ps = (PanelShape)(ps + 1))
                            for(p = 0; p < srf->npanel[ps]; p++) {
                                ...
                                panelnormal(pnl, middle,
                                    face == PFfront ? PFback : PFfront, dim,
...
/* surfsetjumppanel */
int surfsetjumppanel(surfaceptr srf, panelptr pnl1, enum PanelFace face1, int bidirect,
    panelptr pnl2, enum PanelFace face2)
{
    if(!srf)
        return 1;
    if(!pnl1)
...
/* surfaddemitter */
int surfaddemitter(
    surfaceptr srf, enum PanelFace face, int i, double amount, double *pos, int dim)
{
    ...
    if(srf->nemitter[face][i] == srf->maxemitter[face][i]) {  // allocate emitter space if
                                                              // needed
...
/* surfreadstring */
surfaceptr surfreadstring(
    simptr sim, ParseFilePtr pfp, surfaceptr srf, const char *word, char *line2)
{
    ...
    if(!strcmp(word, "name")) {  // name
        ...
        if(ms == MSsoln) {
            ...
            CHECKS(ms1 == ms || ms1 == MSsoln || ms1 == MSbsoln,
                "state1 does not make sense");
        }
#ifdef OPTION_VCELL
        if(found != string::npos) {
            if(constRate == true)
            ...
            string name = rawStr.substr(
                found + 2);  // after the ";" denoting the end of rate, the found move one
                             // more position(the space) to get to the end of the line,
                             // which would be the species name
        ...
        if(!strcmp(word, "rate"))
            er = RuleAddRule(
                sim, RTsurfrate, NULL, pattern, &ms, NULL, f1, detailsi, NULL);
        else {
        ...
        CHECKS(i2 >= 0 && i2 <= 0xFFFF,
            "stipple pattern needs to be between 0x00 and 0xFFFF");
        ...
        face = surfstring2face(facenm);
        CHECKS(face != PFnone,
            "in polygon, face name needs to be 'front', 'back', or 'both'");
        ...
        er = surfsetjumppanel(
            srf, srf->panels[ps][p], face, i1, srf->panels[ps][p2], face2);
                ...
                for(ms1 = (MolecState)0; ms1 < MSMAX; ms1 = (MolecState)(ms1 + 1))
                    for(face = (PanelFace)0; face < 3; face = (PanelFace)(face + 1))
                        if(srf->action[i][ms1][face] == SAmult &&
                            ...
                            actdetails = srf->actdetails[i][ms1][face];
                            for(ms2 = (MolecState)0; ms2 < MSMAX1;
                                ms2 = (MolecState)(ms2 + 1))  // record actual probability
                                                              // of each event
                                actdetails->srfprob[ms2] =
                ...
                for(ms1 = (MolecState)0; ms1 < MSMAX; ms1 = (MolecState)(ms1 + 1))
                    for(face = (PanelFace)0; face < 3; face = (PanelFace)(face + 1)) {
                        if(srf->action[i][ms1][face] == SAmult &&
                            ...
                            sum        = 0;
                            for(ms2 = (MolecState)0; ms2 < MSMAX1;
                                ms2 = (MolecState)(ms2 + 1))  // find ms1->ms1 probability
                            ...
                            for(ms2 = (MolecState)0; ms2 < MSMAX1;
                                ms2 = (MolecState)(ms2 + 1)) {  // record reverse
                                                                // probabilities
        ...
        if(srfss->nmollist) {   // set srfmollist flags
            ...
            for(s = 0; s < srfss->nsrf && checksurfaces == 0; s++) {
                ...
                for(i = 1; i < sim->mols->nspecies && checksurfaces == 0; i++)
                    for(ms = (MolecState)0; ms < MSMAX && checksurfaces == 0;
                        ms = (MolecState)(ms + 1))
                        for(face = (PanelFace)0; face <= PFnone && checksurfaces == 0;
            ...
            for(i = 1; i < sim->mols->nspecies; i++)
                for(ms = (MolecState)0; ms < MSMAX; ms = (MolecState)(ms + 1)) {
                    ...
                    if(checksurfaces && molismobile(sim, i, ms))
                        srfss->srfmollist[ll] =
                            (SMLflag)(srfss->srfmollist[ll] | SMLdiffuse);
                    if(rxnisprod(sim, i, ms, 1))
                        srfss->srfmollist[ll] =
                            (SMLflag)(srfss->srfmollist[ll] | SMLreact);
                    if(ms != MSsoln)
                        srfss->srfmollist[ll] =
                            (SMLflag)(srfss->srfmollist[ll] | SMLsrfbound);
                }
        }
...
/* panelside */
enum PanelFace panelside(
    double *pt, panelptr pnl, int dim, double *distptr, int strict, int useoldpos)
{
    enum PanelFace face;
...
int lineXpanel(double *pt1, double *pt2, panelptr pnl, int dim, double *crsspt,
    enum PanelFace *face1ptr, enum PanelFace *face2ptr, double *crossptr,
    double *cross2ptr, int *veryclose, int useoldpos)
{
    ...
    if(pnl->ps == PSrect) {  // rectangle
        ...
        if(dim == 1)
            ...
            if(d == (int)front[1])
                d = (d + 1) % 3;
            intsct =
                intsct && ((point[1][d] <= crsspt[d] && crsspt[d] <= point[2][d]) ||
        ...
        if(face1 == face2 && (!(nrdist <= point[1][0]) || !(nrpos >= 0) || !(nrpos <= 1)))
        ...
        if(face1 == facein && face1 == face2)
            return 0;
        if(dim == 2)
            cross = Geo_LineXCyl2s(pt1, pt2, point[0], point[1], front, point[2][0],
                &cross2, &nrdist, &nrpos);
        else
            cross = Geo_LineXCyls(
                pt1, pt2, point[0], point[1], point[2][0], &cross2, &nrdist, &nrpos);
        if(face1 == face2 && (!(nrdist <= point[2][0]) || !(nrpos >= 0) || !(nrpos <= 1)))
            return 0;  // negative logic is in case of Nan returns
        if(face1 == facein)
            cross = cross2;
        for(d = 0; d < dim; d++)
...
/* lineexitpanel */
int lineexitpanel(
    double *pt1, double *pt2, panelptr pnl, int dim, double *pnledgept, int *exitside)
{
    int d;
...
/* paneledgenormal */
void paneledgenormal(
    panelptr pnl, double *pnledgept, int dim, int edgenum, double *normal)
{
...
enum SrfAction surfaction(surfaceptr srf, enum PanelFace face, int ident,
    enum MolecState ms, int *i2ptr, enum MolecState *ms2ptr)
{
    enum SrfAction act;
    ...
    if(act == SAmult) {
        ...
        r          = randCOD();
        ms2        = MSnone;
        for(ms3 = (MolecState)0; ms3 < MSMAX1 && ms2 == MSnone;
            ms3 = (MolecState)(ms3 + 1))
            if(r < srfcumprob[ms3])
                ms2 = ms3;
...
/* fixpt2panelnocross */
int fixpt2panelnocross(
    simptr sim, double *pt, panelptr pnl, int dim, enum PanelFace face, double epsilon)
{
        ...
        for(p = 0; p < bptr1->npanel; p++) {
            ...
            for(pn = 0; pn < pnl->nneigh && !ignore; pn++)
                if(testpnl == pnl->neigh[pn])
                    ignore = 1;
            if(!ignore) {
                if(lineXpanel(pt, pt2, testpnl, dim, crsspt, &face1, NULL, &cross, NULL,
                       NULL, 0)) {
...
int closestpanelpt(panelptr pnl, int dim, double *testpt, double *pnlpt, double margin)
{
    ...
    if(pnl->ps == PSrect) {  // rectangle
        ...
        if(dim == 2) {
            edgenum = Geo_NearestLineSegPt(point[0], point[1], testpt, pnlpt, 2, margin);
            dot     = (testpt[0] - point[0][0]) * front[0] +
                  (testpt[1] - point[0][1]) * front[1];
            dot = (dot > 0) ? 1 : -1;
        ...
        if(dim == 2) {
            ...
            edgenum = Geo_NearestDiskPt(
                point[0], front, point[1][0], dim, testpt, pnlpt, margin);
        }
    }
...
/* closestsurfacept */
double closestsurfacept(
    surfaceptr srf, int dim, double *testpt, double *pnlpt, panelptr *pnlptr, boxptr bptr)
{
    enum PanelShape ps;
    int p, d;
...
void movemol2closepanel(simptr sim, moleculeptr mptr)
{
    ...
    if(ptinpanel(mptr->posx, pnlx, dim))  // set mptr->via to starting point that was in
                                          // the panel
        for(d = 0; d < dim; d++)
    ...
    while(iter == 0 || !ptinpanel(mptr->pos, pnl, dim)) {
        iter++;
        er = lineexitpanel(mptr->via, mptr->pos, pnl, dim, pnledgept,
        ...
        if(nn) {  // rotate line to account for new normal vector
            for(d = 0; d < dim; d++)
                mptr->pos[d] +=
                    (newedgept[d] - pnledgept[d]);  // translate position to account for
                                                    // any edge mismatch
            if(dim == 2) {
                if(newedge)
                    thetasign = 1;
                ...
                if(newedge) {  // rotate for trajectory onto the panel edge
                    ...
                    if(fabs(dot) < 0.01) {  // check result if there's any doubt
                        ...
                        if(dot > 0)  // rotated wrong way, so do it again
                            Sph_RotateVectWithNormals3D(newedgept, mptr->pos, newpos,
                                oldnormal, newnormal, -thetasign);
                    }
                    for(d = 0; d < 3; d++)
                        mptr->pos[d] = newpos[d];
                }
    ...
    moleculeptr mptr, panelptr pnl, double *crsspt, int dim, enum PanelFace face)
{
    ...
    if(mptr->mstate == MSsoln) {
        ...
        panelnormal(mptr->pnlx, crsspt, PFfront, dim,
...
#ifdef VCELL
                surfUpdateRate(sim, mptr, facemin,
                    pnlmin);  // this is a very inefficient function.  If we defined
                              // suface rate as function then we have to evaluate the rate
                              // every time step when surface activity happens
#endif
                done = dosurfinteract(sim, mptr,
                    sim->mols->listtype[mptr->list] == MLTsystem ? mptr->list : -1, -1,
                    pnlmin, facemin, crssptmin);
                for(d = 0; d < dim; d++)
                    via[d] = crssptmin[d];
        ...
        while(!done) {
            ...
            for(bptr1 = pos2box(sim, via); bptr1;
                ...
                for(p = 0; p < bptr1->npanel; p++) {
                    pnl = bptr1->panel[p];
                    if(pnl != mptr->pnl) {
                        lxp = lineXpanel(via, pos, pnl, dim, crsspt, &face, NULL, &cross,
                            NULL, NULL, 0);
