...
unsigned long long molfindserno(simptr sim, unsigned long long def, long int pserno,
    unsigned long long r1serno, unsigned long long r2serno,
...
// structure set up
int RuleAddRule(simptr sim, enum RuleType type, const char *rname, const char *pattern,
    const enum MolecState *rctstate, const enum MolecState *prdstate, double rate,
...
void surftransformpanel(panelptr pnl, int dim, double *translate, double *origin,
    double *expand);  //?? not documented
...
int closestpanelpt(
    panelptr pnl, int dim, double *testpt, double *pnlpt, double neighdist);
...
compartptr compartreadstring(
    simptr sim, ParseFilePtr pfp, compartptr cmpt, const char *word, char *line2);
...
portptr portaddport(
    simptr sim, const char *portname, surfaceptr srf, enum PanelFace face);
portptr portreadstring(
    simptr sim, ParseFilePtr pfp, portptr port, const char *word, char *line2);
...
int portgetmols2(simptr sim, portptr port, int ident, enum MolecState ms, int remove,
    double **positions);
...
filamentptr filreadstring(
    simptr sim, ParseFilePtr pfp, filamentptr fil, const char *word, char *line2);
...
double evaluateSurfActionRate(simptr sim, surfactionptr actdetails, enum MolecState ms,
    double *pos, char *panelName);
