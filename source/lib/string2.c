...
#define CHECKS(A, ...)                                                 \
    if(!(A)) {                                                         \
        snprintf(StrErrorString, sizeof(StrErrorString), __VA_ARGS__); \
        goto failure;                                                  \
    }                                                                  \
    else                                                               \
        (void)0
...
/* strPbrkBrackets */
int strPbrkBrackets(
    const char *string, int n, const char *c, const char *delimit, int imin)
{
    int  i, pn, bk, bc, qt, sq, ckpn, ckbk, ckbc, ckqt, cksq;
    char ch;
...
int wordcountpbrk(const char *s, const char *symbols)
{
    ...
    ischar = 0;
    for(s2 = s; *s2; s2++) {
        if(sp && (ischar = !strchr(symbols, *s2)))
            n++;
        sp = !ischar;
    }
    return n;
...
char *strMidCat(char *str, int s1, int s2, const char *cat, int start, int stop)
{
    ...
    len   = strlen(str);
    if(shift > 0)
        for(i = len + shift; i >= s2 + shift; i--)
            str[i] = str[i - shift];
    else if(shift < 0)
        for(i = s2 + shift; i <= len + shift; i++)
            str[i] = str[i - shift];
    for(i = 0; i < n; i++)
        str[i + s1] = cat[start + i];
    return str;
}
    ...
    for(p = 0; p < n; p++) {
        if(pat[p] == '-') {  // initial -
            if(p + 1 == n)
                return 1;  // only -
            if(ch <= pat[p + 1])
                return 1;  // match initial -
            p++;
        }
        else if(p + 1 < n && pat[p + 1] == '-') {  // internal or terminal -
            if(p + 2 == n) {                       // terminal -
                if(ch >= pat[p])
                    return 1;  // terminal - match
                return 0;
            }  // terminal - no match
            if(ch >= pat[p] && ch <= pat[p + 2])
...
int strwildcardmatch(const char *pat, const char *str)
{
    ...
    while(str[s]) {
        if(pat[p] == '*') {  // star found in pat
            if(!pat[p + 1])
                return 1;  // terminal star, so this is a match
            p++;
            p1 = p;  // p and p1 are at '*'+1, where post-star text will resume
            s1 = s;
        }  // s1 is at s, where any star text will start
        else if(pat[p] == '?') {
            s++;
            p++;
        }                         // '?' wildcard character
            ...
            if(match) {
                ...
                goto starlabel;  // act as if this option weren't ever selected
            else
                return 0;
        }
        else if(pat[p] == str[s]) {
...
int strwildcardmatchandsub(const char *pat, const char *str, char *dest, int starextra)
{
    ...
    while(str[s]) {  // walk through pat and str, determining if match and
        ...
        if(pat[p] == '*') {
            ...
            if(match) {
                ...
                return 0;
        }  // bracket pattern doesn't match and no open star, so no match
        ...
        starlabel:
            ...
            else {  // str and pat did match but now don't, so try one larger
                    // star text
                isub = isubstar + 1;
                p    = p1;
                s1++;
    ...
    if(starxc < starextra)
        return 0;  // doesn't qualify as match because extra characters weren't
                   // used
    ...
    if(pat[p])
        return 0;  // more text remains in pat, so no match
    nsub = isub;
    ...
    if(isym >= start) {  // AND operator found at i2
        ...
        for(il = 0; il < lnum; il++)
            for(ir = 0; ir < rnum; ir++) {
                ires          = il * rnum + ir;
                results[ires] = (char *)calloc(
                    strlen(llist[il]) + strlen(rlist[ir]) + 1, sizeof(char));
                if(!results[ires])
                    return -1;
                strcpy(results[ires], llist[il]);
                strcat(results[ires], rlist[ir]);
...
int strEnhWildcardMatch(const char *pat, const char *str)
{
    ...
    if(str)
        for(i = 0; i < nr; i++)
            if(strwildcardmatch(results[i], str))
                return 1;
...
int strEnhWildcardMatchAndSub(
    const char *pat, const char *str, const char *destpat, char *dest)
{
    ...
    if(!pat || !localpat || strcmp(pat, localpat)) {
        if(npr > 0) {  // npr is number of pattern results, which is items in
                       // patlist
            for(i = 0; i < npr; i++)
                free(patlist[i]);
            free(patlist);
        ...
        if(pat) {
            ...
            npr = strexpandlogic(localpat, 0, -1,
                &patlist);  // patlist lists the same pattern as pat but with
                            // the logic expanded
            if(npr < 0)
                return npr;
        }               // error occured
    ...
    if(!destpat || !localdestpat || strcmp(destpat, localdestpat)) {
        if(ndr > 0) {  // ndr is number of destination results, which is items
                       // in destlist
            for(i = 0; i < ndr; i++)
                free(destlist[i]);
            free(destlist);
        ...
        if(destpat) {
            ...
            ndr = strexpandlogic(localdestpat, 0, -1,
                &destlist);  // destlist lists the same pattern as destpat but
                             // with logic expanded
            if(ndr < 0)
                return ndr;
        }
    ...
    if((ndr == 0 || ndr == 1) && npr > 0) {
        while(iresults < npr) {  // iresults scans of list of patterns
            if(strwildcardmatch(patlist[iresults], str)) {
                ...
                if(ismatch && wordcount(dest) == destwords)
                    return 1;
            }
            else {
                starextra = 0;
        ...
        if(patlist ? strwildcardmatch(patlist[0], str) : str[0] == '\0') {
            while(iresults < ndr) {  // iresults scans of list of destinations
                ...
                ismatch           = strwildcardmatchandsub(
                    patlist ? patlist[0] : "", str, dest, starextra);
                if(ismatch)
                    starextra++;  // starextra looks for multiple matches when
                                  // there are 2 stars
                else {
...
int strloadmathfunctions(void)
{
    ...
    er += strevalfunction(strcpy(str1, "acos"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &acos), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "asin"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &asin), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "atan"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &atan), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "cos"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &cos), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "cosh"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &cosh), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "sin"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &sin), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "sinh"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &sinh), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "tan"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &tan), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "tanh"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &tanh), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "exp"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &exp), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "log"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &log), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "log10"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &log10), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "sqrt"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &sqrt), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "ceil"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &ceil), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "fabs"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &fabs), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "floor"), strcpy(str2, "dd"), NULL,
        (void *)(fnptrdd = &floor), NULL, NULL, 0);

    er += strevalfunction(strcpy(str1, "atan2"), strcpy(str2, "ddd"), NULL,
        (void *)(fnptrddd = &atan2), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "pow"), strcpy(str2, "ddd"), NULL,
        (void *)(fnptrddd = &pow), NULL, NULL, 0);
    er += strevalfunction(strcpy(str1, "rand"), strcpy(str2, "ddd"), NULL,
        (void *)(fnptrddd = &unirandCCD), NULL, NULL, 0);
    ...
    char **varnames, const double *varvalues, int nvar)
{
    ...
    for(i = 0; i < nfunc && strcmp(expression, funclist[i]); i++)
    ...
    if(!strcmp(paramlist[i], "dd")) {
        ...
        CHECKS((comma = strChrBrackets(parameters, -1, ',', "([{,\"'")) > 0,
            "missing parameter");
        parameters[comma] = '\0';
        f1                = strmatheval(parameters, varnames, varvalues, nvar);
        parameters += comma + 1;
...
double strmatheval(char *expression, char **varnames, const double *varvalues, int nvar)
{
    ...
    if(strisnumber(expression)) {  // number
    ...
    else if(strchr("([{", expression[0]) &&
            strparenmatch(expression, 0) == length - 1) {  // {[()]}
        unarysymbol = 0;
        ...
        *ptr        = ptrchar;
        ...
        CHECKS(answer > 0 || (answer == 0 && term > 0) ||
                   (answer < 0 && term == round(term)),
            "exponent error");
...
/* strmathsscanf */
int strmathsscanf(const char *str, const char *format, char **varnames,
    const double *varvalues, int nvar, ...)
{
    va_list     arguments;
    ...
    while(fmtpos2) {
        if(*(fmtpos2 + 2) == 'i')
            readint = 1;
        else if(*(fmtpos2 + 2) == 'l' && *(fmtpos2 + 3) == 'g')
            readint = 0;
        else
    ...
    if(!strmatherror(NULL, 0)) {
        if(fmtpos1)
            strcat(newformat, fmtpos1);
        if(strpos1)
            strcat(newstr, strpos1);
    }
