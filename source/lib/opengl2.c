...
int WriteTIFF(const char *filename, const char *description, int x, int y,
...
void KeyPush(unsigned char key, int x, int y)
{
    ...
    if(key == 'Q') {
        ...
        w = viewport[2];
        h = viewport[3];
        snprintf(str, STRCHAR, "%s%%0%ii.tif", TiffName,
        ...
        if(Fix2DAspect) {
            ...
            glOrtho(ClipLeft, ClipLeft + clipwidth, ClipBot,
                    ClipBot + clipheight, ClipFront, ClipBack);
        }
        else
            glOrtho(ClipLeft, ClipRight, ClipBot, ClipTop, ClipFront, ClipBack);
        glMatrixMode(GL_MODELVIEW);
        ...
        if(key == '0') {
            ...
            if(w >= h)
                ...
                Near = ClipSize / 2.0 /
                       tan(FieldOfView * Aspect * PI / 180.0 / 2.0);
            glMatrixMode(GL_PROJECTION);
            glLoadIdentity();
            gluPerspective(FieldOfView, Aspect, Near, ClipSize + Near);
            glMatrixMode(GL_MODELVIEW);
...
/* gl2Initialize */
void gl2Initialize(char *wname, float xlo, float xhi, float ylo, float yhi,
                   float zlo, float zhi)
{
#ifdef __gl_h_
    if(ylo == yhi && zlo == zhi)
        ...
        Dimension = 3;
    ClipSize =
        1.05 * sqrt((xhi - xlo) * (xhi - xlo) + (yhi - ylo) * (yhi - ylo) +
                    (zhi - zlo) * (zhi - zlo));
    if(ClipSize == 0)
        ClipSize = 1.0;
...
void gl2glutInit(int *argc, char **argv)
{
    ...
    if(argc && argv)
        glutInit(argc, argv);  // ?? This is where the code crashes when using
                               // freeglut
    else {
...
void gl2SetColor(char c)
{
    ...
    if(c == 'A' || c == 'a')
        ...
        glColor3ub(intrand(2) * 0xff, intrand(2) * 0xff,
                   intrand(2) * 0xff); /* random	*/
    else if(c == '1')
...
void gl2DrawCircle(float *cent, float radius, int slices, char style, int dim)
{
    ...
    if(dim == 2)
        for(i = 0; i < slices; i++) {
            theta = i * dtheta;
            glVertex2f(cent[0] + radius * cos(theta),
                       cent[1] + radius * sin(theta));
        }
    else
        for(i = 0; i < slices; i++) {
            theta = i * dtheta;
            glVertex3f(cent[0] + radius * cos(theta),
                       cent[1] + radius * sin(theta), cent[2]);
        }
    if(style == 'e' || style == 'f') {
        theta = i * dtheta;
        if(dim == 2)
                    ...
                    int dim)
{
    ...
    if(dim == 2)
        for(i = 0; i < slices; i++) {
            theta = i * dtheta;
            glVertex2d(cent[0] + radius * cos(theta),
                       cent[1] + radius * sin(theta));
        }
    else
        for(i = 0; i < slices; i++) {
            theta = i * dtheta;
            glVertex3d(cent[0] + radius * cos(theta),
                       cent[1] + radius * sin(theta), cent[2]);
        }
    if(style == 'e' || style == 'f') {
        theta = i * dtheta;
...
/* gl2DrawArc */
void gl2DrawArc(float *cent, float radius, float theta1, float theta2,
                int slices, char style, int dim)
{
#ifdef __gl_h_
    int i, imax;
    float theta, dtheta;
...
/* gl2DrawArcD */
void gl2DrawArcD(double *cent, double radius, double theta1, double theta2,
                 int slices, char style, int dim)
{
#ifdef __gl_h_
    int i, imax;
    ...
    for(ilat = 1; ilat < stacks; ilat++) {
        ...
        if(!normals)
            ...
            for(ilong = ilong1; ilong != ilong2; ilong += dilong) {
                cosphi = cos(ilong * dphi);
                sinphi = sin(ilong * dphi);
                glNormal3f(r1 * normfact * cosphi, r1 * normfact * sinphi,
                           z1 * normfact);
                glVertex3f(r1 * cosphi, r1 * sinphi, z1);
                glNormal3f(r2 * normfact * cosphi, r2 * normfact * sinphi,
                           z2 * normfact);
                glVertex3f(r2 * cosphi, r2 * sinphi, z2);
            }
        glEnd();
...
/* gl2DrawCylinder */
void gl2DrawCylinder(float baseRadius, float topRadius, float height,
                     int slices, int stacks, int frontin, int normals)
{
#ifdef __gl_h_
    ...
    if(baseRadius == topRadius) {
        ...
        normxy = (frontin ? -1.0 : 1.0) /
                 sqrt((baseRadius - topRadius) * (baseRadius - topRadius) +
                      height * height);
...
/* gl2DrawEcoli */
void gl2DrawEcoli(float radius, float length, int slices, int stacks,
                  int frontin, int normals)
{
#ifdef __gl_h_
    int endstacks;
    ...
    if(length)
        gl2DrawCylinder(radius, radius, length, slices, stacks - 2 * endstacks,
                        frontin, normals);
    glRotatef(180, 1, 0, 0);
    gl2DrawHemisphere(radius, slices, endstacks, frontin, normals);
    glPopMatrix();
...
/* gl2DrawTextD */
void gl2DrawTextD(double x, double y, double *color, void *font, char *string,
                  int align)
{
#ifdef __gl_h_
    int i, width, length;
...
void gl2PlotData(float *xdata, float *ydata, int nx, int nycol, char *style)
{
    ...
    for(j = 0; j < nycol; j++) {
        if(style[3 * j] == ' ')
            ...
            if(nx > 1) {
                glVertex3f(xdata[i], ydata[i * nycol + j], 0);
                glVertex3f(2 * xdata[i] - xdata[i - 1], ydata[i * nycol + j],
                           0);
...
/* gl2PlotPts */
void gl2PlotPts(float **data, int *ser, int nser, int npts, float **color,
                float *size, char style)
{
#ifdef __gl_h_
    int i, j;
