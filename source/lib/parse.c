...
#define CHECKS(A, ...)                         \
    if (!(A)) {                                \
        snprintf(erstr, STRCHAR, __VA_ARGS__); \
        goto failure;                          \
    }                                          \
    else                                       \
        (void)0
...
ParseFilePtr Parse_AllocFilePtr(const char *fileroot, const char *filename)
{
    ...
    if (filename)
        strncat(pfp->fname, filename, STRCHAR - 1 - strlen(pfp->fname));
...
void Parse_FreeFilePtr(ParseFilePtr pfp)
{
    ...
    if (pfp->maxdef) {
        if (pfp->defreplace)
            for (d = 0; d < pfp->maxdef; d++) free(pfp->defreplace[d]);
        if (pfp->defkey)
            for (d = 0; d < pfp->maxdef; d++) free(pfp->defkey[d]);
    }
    free(pfp->defgbl);
    free(pfp->defreplace);
    free(pfp->defkey);
...
failure:
    if (newdefrep)
        for (d = pfp->ndef; d < maxdef; d++) free(newdefrep[d]);
    if (newdefkey)
        for (d = pfp->ndef; d < maxdef; d++) free(newdefkey[d]);
    free(newdefgbl);
    free(newdefrep);
    free(newdefkey);
    return 1;
...
/* Parse_AddDefine. */
int Parse_AddDefine(ParseFilePtr pfp, const char *key, const char *replace,
                    int global)
{
    int d, d2, er;
    unsigned int len;
    ...
    if (replace)
        strncpy(pfp->defreplace[d], replace, STRCHAR - 1);
    else
        pfp->defreplace[d][0] = '\0';
...
int Parse_DoDefine(ParseFilePtr pfp)
{
    ...
    total = 0;
    for (d = 0; d < pfp->ndef; d++) {
        val = strstrreplace(line2, pfp->defkey[d], pfp->defreplace[d],
                            STRCHAR - offset);
        if (val < 0)
            ans = 2;
        else
...
/* Parse_Start. */
ParseFilePtr Parse_Start(const char *fileroot, const char *filename,
                         char *erstr)
{
    ParseFilePtr pfp;
    char string[STRCHAR];
...
/* Parse_ReadLine. */
int Parse_ReadLine(ParseFilePtr *pfpptr, char *word, char **line2ptr,
                   char *erstr)
{
    ...
    CHECKS(pfpptr && word && line2ptr && erstr,
           "BUG: Parse_ReadLine missing parameters");
    pfp = *pfpptr;
    if (!pfp) return 2;
    ...
    if (linetest) {  // pre-process line
        ...
        if (pfp->incomment) {
            skip = 1;
            toolong = 0;
            if (!strncmp(line, "*/", 2)) pfp->incomment = 0;
        }
        if (!skip && pfp->inifdef) {
            skip = 1;
            if (!strncmp(line, "ifdefine", 8))
                pfp->inifdef++;
            else if (!strncmp(line, "endif", 5))
                pfp->inifdef--;
            else if (pfp->inifdef == 1 && !strncmp(line, "else", 4))
                pfp->inifdef = 0;
        }
        if (!skip) {
            CHECKS(!toolong,
                   "Line exceeds maximum allowable length of %i characters",
                   STRCHAR);
    ...
    if (skip)
        ;
             ...
             (itct > 0 && !strcmp(word, "end_file"))) {  // end_file
        pfp1 = pfp->prevfile;
        fclose(pfp->fptr);
        Parse_FreeFilePtr(pfp);
        pfp = pfp1;
        *pfpptr = pfp;
        ...
        for (d = 0; d < pfp->ndef; d++)
            if (pfp->defgbl[d]) {
                CHECKS(!Parse_AddDefine(pfp1, pfp->defkey[d],
                                        pfp->defreplace[d], 1),
                       "unable to allocate memory for global defines");
            }
        pfp = pfp1;
        *pfpptr = pfp;
        pfp->fptr = fopen(pfp->fname, "r");
        ...
        if (er == 2)
            printf(
                "Warning: 'define %s %s' is ignored because it is a "
                "re-definition\n",
                str1, line2);
        ...
        if (strcmp(str1, "all"))
            Parse_RemoveDefine(pfp, str1);
        else
            Parse_RemoveDefine(pfp, NULL);
    }
        ...
        itct = sscanf(line2, "%s", str1);
        CHECKS(itct == 1, "unable to read undefine key");
        if (stringfind(pfp->defkey, pfp->ndef, str1) == -1) pfp->inifdef = 1;
    }
